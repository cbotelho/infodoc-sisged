...
class fields
{
    ...
    {
        $cache = array();
        $fields_query = db_query("select id, type, IFNULL(name,'') as name, entities_id, configuration, is_heading from app_fields");
        while($fields = db_fetch_array($fields_query))
        {
    ...
    public static function get_choices($entities_id, $cfg=[])
    {
        ...
        if($cfg->get('include_parents')==true)
        {
            ...
            foreach($parents as $entity_id)
            {
                ...
                {
                    $choices[$app_entities_cache[$entity_id]['name']][$v['id']] = fields_types::get_option($v['type'], 'name', $v['name']);
                }                
            }
        }
        else
    ...
    public static function check_before_delete($entity_id, $id)
    {
        ...
        if(strlen($msg))
        {
            ...
            $msg = sprintf(TEXT_YOU_CANT_DELETE_FIELD,$name) . '<br><br><p class="alert alert-warning">' . $msg . '</p>';
        }
    ...
    public static function render_unique_fields_ruels($entities_id, $item_id = false)
    {
        ...
        {
            $url = url_for("items/items", "action=check_unique&path=" . $app_path . ($item_id ? "&id=" . $item_id : ""));
        }

        $html = '';

        $fields_query = db_query("select f.* from app_fields f where  f.entities_id='" . db_input($entities_id) . "' order by f.sort_order, f.name");
        while($v = db_fetch_array($fields_query))
        {
        ...
        //add composite uniqure rule check
        $html .= composite_unique_fields::add_rule($entities_id, $item_id);
    ...
    public static function get_filters_choices($entity_id, $show_parent_item_fitler = true, $exclude = "")
    {
        ...
        $choices = array();
        $choices[''] = '';
        $fields_query = db_query("select f.*, t.name as tab_name, if(f.type in ('fieldtype_id','fieldtype_date_added','fieldtype_date_updated','fieldtype_created_by','fieldtype_parent_item_id'),-1,t.sort_order) as tab_sort_order from app_fields f, app_forms_tabs t where f.type in (" . $types_for_filters_list . ") " . (strlen($exclude) ? " and f.type not in ({$exclude})" : '') . " and f.entities_id='" . db_input($entity_id) . "' and f.forms_tabs_id=t.id order by tab_sort_order, t.name, f.sort_order, f.name");
        while($v = db_fetch_array($fields_query))
        {
            //check field access
            if(isset($fields_access_schema[$v['id']]))
    ...
    public static function check_if_type_changed($field_id, $new_type)
    {
        ...
        if($field_info = db_fetch_array($field_info_query))
        {
            ...
            if($field_info['type'] != $new_type and !self::check_if_type_is_similar($field_info['type'], $new_type))
            {
                //delete index
    ...
    public static function check_if_type_is_similar($current_type, $new_type)
    {
        $type_groups = [];
        ...
        {
            if(in_array($current_type,$types) and in_array($new_type,$types))
            {
                return true;
            }
        }
    ...
    public static function get_items_fields_data_by_id($item, $fields_list = '', $entities_id = 0, $fields_access_schema = [])
    {
    ...
    public static function get_items_fields_fresh_data($item, $fields_list = '', $entities_id = 0, $fields_access_schema = [])
    {
        global $app_choices_cache, $app_users_cache;
    ...
    static function get_available_fields_helper($entities_id, $template_field_id, $dropdown_title = TEXT_AVAILABLE_FIELDS, $use_fieldtypes = [], $skip_reserved = false, $include_parent = false)
    {
        global $app_entities_cache;
            ...
            $html .= '  			  			
	  	    ...
	  	    <li>
	  	      <a href="#" class="insert_to_template_' . $unique_id . '" data-field="[date_added]">' . TEXT_FIELDTYPE_DATEADDED_TITLE . ' [date_added]</a>
                    ...
                    <li>
	  	      <a href="#" class="insert_to_template_' . $unique_id . '" data-field="[date_updated]">' . TEXT_FIELDTYPE_DATE_UPDATED_TITLE . ' [date_updated]</a>
	  	    </li>
	  	    <li>
        ...
        if($include_parent and $entities_info['parent_id']>0)
        {
            ...
            $fields_query = fields::get_query($entities_info['parent_id'], "and f.type not in (" . fields_types::get_reserverd_types_list() . ")");
        ...
        $html .= '
  			...
  			$(".insert_to_template_' . $unique_id . '").click(function(){
                                ...
                                {
                                    editor = myCodeMirror' . str_replace('fields_configuration_','',$template_field_id). ' 
    ...
    static function get_value_by_type($entity_id, $field_id, $field_value)
    {
        global $app_fields_cache, $app_users_cache;
        
        if(!$field = $app_fields_cache[$entity_id][$field_id]??false)
        {
            return '';
        }
        ...
        switch($field['type'])
        {
            case 'fieldtype_input_date':
                $field_value = format_date($field_value, $cfg->get('date_format'));
                break;
            case 'fieldtype_input_datetime':
                $field_value = format_date_time($field_value, $cfg->get('date_format'));
                break;
            case 'fieldtype_input_date_extra':
                $field_value = format_date($field_value, fieldtype_input_date_extra::get_date_format($cfg));
                break;
            ...
            case 'fieldtype_color':    
                if($cfg->get('use_global_list')>0)
                {
                  $field_value = global_lists::render_value($field_value, true);
                }
                else
                {
            ...
            case 'fieldtype_users':                
                if(is_array($field_value) and count($field_value))
                {
                    ...
                    $field_value = implode(', ', $users_list);
                }
                break;
            ...
            case 'fieldtype_entity_multilevel':
                ...
                $field_value = is_array($field_value) ? $field_value : (strlen($field_value) ? explode(',',$field_value) : []);  
                                                
                $output = array();
                foreach ($field_value as $item_id)
                {
                    $items_info_sql = "select e.* from app_entity_" . $cfg->get('entity_id') . " e where e.id='" . db_input($item_id) . "'";
