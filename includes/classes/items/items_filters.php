...
class items_filters
{
    ...
    function check($cfg=[])
    {
        ...
        $reports_info_query = db_query("select id from app_reports where entities_id='" . db_input($this->entity_id) . "' {$where_sql}");
        if ($reports_info = db_fetch_array($reports_info_query))
        {
            ...
            $item_info_sql = "select e.id " . fieldtype_formula::prepare_query_select($this->entity_id) . " from app_entity_" . $this->entity_id . " e  where e.id='" . $this->item_id . "' " . $listing_sql_query;
