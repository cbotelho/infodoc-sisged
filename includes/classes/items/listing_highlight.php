...
class listing_highlight
{
    ...
    function __construct($entities_id)
    {
        ...
        $fields_query = db_query("select r.*, f.name, f.type, f.configuration from app_listing_highlight_rules r, app_fields f where r.is_active=1 and (length(r.users_groups)=0 or find_in_set({$app_user['group_id']},r.users_groups)) and f.id = r.fields_id and r.entities_id='" . $this->entities_id . "' order by r.sort_order, r.id");
    ...
    static function get_field_value_by_type($field, $value)
    {
        ...
        switch (self::get_field_type_key($field['type']))
        {
            ...
            case 'entities':
                if(strlen($value))
                {
                    $items_info_sql = "select e.* from app_entity_" . $cfg->get('entity_id') . " e where e.id in (" . db_input_in($value) . ")";
