...
class items_listing
{
    ...
    function __construct($reports_id, $entity_cfg = false)
    {
        ...
        if((!strlen($this->fields_in_listing) and is_array($this->settings->get('fields_in_listing'))) or  (is_array($this->settings->get('fields_in_listing')) and !$this->is_listing_fields_configuration()))
        {
            $this->fields_in_listing = implode(',', $this->settings->get('fields_in_listing'));            
        }                        
        elseif(!$this->is_listing_fields_configuration())
        {
    ...
    function is_listing_fields_configuration()
    {
        ...
        if(strlen($this->entity_cfg->get('disable_listing_fields_configuration')) and in_array($this->reports_info['reports_type'],['entity','entity_menu','standard']))
        {            
    ...
    function select_all_choices($number_of_pages)
    {
        ...
        {
            $html = input_checkbox_tag('select_all_items', $this->reports_info['id'], array('class' => 'select_all_items'));
        }
        elseif($number_of_pages>1)
        {
            ...
            $html = '
                <span class="hidden">' . input_checkbox_tag('select_all_items', $this->reports_info['id'], array('class' => 'select_all_items')) . '</span>
                <span class="hidden">' . input_checkbox_tag('select_all_items', $this->reports_info['id'], array('class' => 'select_all_items_current_page')) . '</span>    
                <div class="btn-group">
