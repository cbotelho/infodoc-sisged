...
class ldap_login
{
  ...
  {  	
  	...
  	$search = @ldap_search(
  		...
  		0,
  		1
  	) or die("Error in ldap_search query: " . ldap_error($ldap) . '<br>base: ' . htmlspecialchars_decode($this->config['ldap_base_dn']) . '<br>filter: ' . $this->ldap_user_filter($username) . '<br>attributes: ' . json_encode($search_filters));
