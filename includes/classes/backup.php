...
class backup
{
    ...
    function create()
    {
        ...
        //add description
        $description = 'Rukovoditel ' . PROJECT_VERSION . (strlen($this->description) ? "\n" . $this->description : '');
        foreach (preg_split('/\r\n|\r|\n/', $description) as $text)
        {
            fwrite($fp, "#" . $text . ";\n");
        }
    ...
    {
        if(CFG_AUTOBACKUP_KEEP_FILES_DAYS==0) return false;
    ...
    {
        return ($is_auto ? 'autobackup' : $id) . '_' . date('Y-m-d_H-i', $timestamp) . '_Rukovoditel_' . PROJECT_VERSION . '.sql';
    }
    ...
    static function reset($is_auto = false)
    {
        ...
        while ($file = $dir->read())
        {
            if (!is_dir($backup_dir . $file) and $file != '.htaccess' and (substr($file, -4) == '.zip' or substr($file, -4) == '.sql'))
            {
                $count_query = db_query("select count(*) as total from app_backups where filename='" . db_input($file) .  "' and is_auto='{$is_auto}'");
                $count = db_fetch_array($count_query);
                $count = $count['total'];
