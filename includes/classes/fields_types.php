...
class fields_types
{
    ...
    public static function get_types_for_assigned_users()
    {
        return [
            'fieldtype_users',
    ...
    public static function get_types_excluded_in_form()
    {
        return array(
            ...
            'fieldtype_yandex_map_directions',
            'fieldtype_yandex_map_nested',
    ...
    public static function get_types_for_assigned_users_list()
    {
        return "'" . implode("','", fields_types::get_types_for_assigned_users()) . "'";
    }
    ...
    public static function get_tooltip($fieldtype)
    {
        ...
        switch ($fieldtype)
        {
            ...
            case 'fieldtype_input_date_extra':
                $tooltip = TEXT_FIELDTYPE_INPUT_DATE_EXTRA_TOOLTIP;
                break;
            ...
            case 'fieldtype_yandex_map':
                ...
                break;      
            case 'fieldtype_yandex_map_directions':
                $tooltip = TEXT_FIELDTYPE_YANDEX_MAP_DIRECTIONS_TOOLTIP ;
                break;
            case 'fieldtype_yandex_map_nested':
                $tooltip = TEXT_FIELDTYPE_YANDEX_MAP_NESTED_TOOLTIP ;
                break;
            ...
            case 'fieldtype_onlyoffice':
                $tooltip = TEXT_FIELDTYPE_ONLYOFFICE_TOOLTIP;
                break;
            case 'fieldtype_onlyoffice':
                $tooltip = TEXT_FIELDTYPE_ONLYOFFICE_TOOLTIP;
                break;
            case 'fieldtype_dropbox':
                $tooltip = TEXT_FIELDTYPE_DROPBOX_TOOLTIP;
                break;
            case 'fieldtype_yandex_disk':
                $tooltip = TEXT_FIELDTYPE_YANDEX_DISK_TOOLTIP;
                break;           
            case 'fieldtype_google_drive':
                $tooltip = TEXT_FIELDTYPE_GOOGLE_DRIVE_TOOLTIP;
                break;
    ...
    public static function get_choices()
    {
        ...
        $fieldtypes[TEXT_FILE_STORAGE] = array(
            'fieldtype_dropbox',  
    ...
    public static function render_field_name($name, $class, $fields_id)
    {
        ...
        elseif(in_array($class, array('fieldtype_image_map_nested','fieldtype_entity_ajax', 'fieldtype_entity_multilevel')))
        {
        ...
        foreach ($tabs_cfg as $tab_id => $cfg)
        {
            ...
            foreach ($cfg as $tab_name => $v)
            {
	            ...
	            ';
                        break;
                    case 'text':
                        $field = '<p class="form-control-static">' . $v['html'] . '</p>';
                        break;
                    case 'input':
                        ...
                        break;
                    case 'input_readonly':
                        $v['params']['readonly']='readonly';
                        $field = input_tag('fields_configuration[' . $v['name'] . ']', $v['default'], (isset($v['params']) ? $v['params'] : array()));
                        break;
                    case 'input_icon':
                        $field = input_icon_tag('fields_configuration[' . $v['name'] . ']', (isset($configuration[$v['name']]) ? $configuration[$v['name']] : ''), (isset($v['params']) ? $v['params'] : array()));
                        break;
                    case 'file':
	        ...
	        <div class="form-group from_group_' . $v['name'] . '" ' . ((isset($v['form_group']) and is_array($v['form_group'])) ? tag_attributes_to_html($v['form_group']) : '') . '>
    ...
    {
        if (is_null($class) or !strlen($class))
            return '';
    ...
    public static function update_items_fields($current_entity_id, $item_id)
    {
        ...
        fieldtype_yandex_map_directions::update_items_fields($current_entity_id, $item_id, $item_info);
    ...
    {
        if (strlen($value) == 0 or ($value == 0 and in_array($type, ['fieldtype_dropdown', 'fieldtype_radioboxes', 'fieldtype_created_by', 'fieldtype_input_date','fieldtype_input_date_extra', 'fieldtype_input_datetime', 'fieldtype_time', 'fieldtype_entity_multilevel'])))
        {
            return true;
        } else
        {
