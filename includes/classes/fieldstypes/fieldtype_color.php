...
class fieldtype_color
{
    ...
    {
        $this->options = array('title' => TEXT_FIELDTYPE_COLOR_TITLE, 'has_choices' => true);
    }
    ...
    function get_configuration()
    {
        ...
        $cfg[] = array('title' => TEXT_DISPLAY_USERS_AS,
            'name' => 'display_as',
            'type' => 'dropdown',
            'choices' => array('dropdown' => TEXT_DISPLAY_USERS_AS_DROPDOWN, 'dropdown_muliple' => TEXT_DISPLAY_USERS_AS_DROPDOWN_MULTIPLE, 'checkboxes' => TEXT_DISPLAY_USERS_AS_CHECKBOXES,),
        ...
        $cfg[] = array(
            'title'=>TEXT_COLUMN, 
            'name'=>'ul-class',
            'type'=>'dropdown','choices'=>fieldtype_radioboxes::get_display_as_choices(),
    ...
    function render($field, $obj, $params = array())
    {
        ...
        {
            $choices = global_lists::get_choices_with_color($cfg->get('use_global_list'), (($field['is_required'] == 1 and !strlen($cfg->get('default_text'))) ? false : true), '', $obj['field_' . $field['id']], true);
            $default_id = global_lists::get_choices_default_id($cfg->get('use_global_list'));
        }
        else
        {
            $choices = fields_choices::get_choices_with_color($field['id'], (($field['is_required'] == 1 and !strlen($cfg->get('default_text'))) ? false : true), '', $cfg->get('display_choices_values'), $obj['field_' . $field['id']], true);
        ...
        switch($cfg->get('display_as'))
        {
            ...
            case 'checkboxes':   
                ...
                if(isset($choices[''])) unset($choices['']);
                
                $html = '<div class="checkbox-list ' . ($attributes['ul-class']=='list-inline' ? ' form-control-static':'') . '">' . select_checkboxes_ul_color_tag('fields[' . $field['id'] . ']',$choices,$value,$attributes) . '</div>';
