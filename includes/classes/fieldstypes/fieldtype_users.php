...
class fieldtype_users
{
    ...
    function render($field, $obj, $params = array())
    {
        ...
        if ($cfg->get('display_as') == 'dropdown')
        {
            ...
            $attributes = array('class' => 'form-control chosen-select2 input-large field_' . $field['id'] . ($field['is_required'] == 1 ? ' required' : ''));
        ...
        {
            $attributes = array('class' => 'form-control input-xlarge chosen-select2 field_' . $field['id'] . ($field['is_required'] == 1 ? ' required' : ''),
                'multiple' => 'multiple',
    ...
    static function get_heading_field($item_id)
    {
        $users_query = db_query("select * from app_entity_1 where id='" . $item_id. "'");
    ...
    function output($options)
    {
        ...
        if (isset($options['is_export']))
        {
            ...
            foreach (explode(',', $options['value']??'') as $id)
            {
                ...
                {
                    $users_list[] = self::get_heading_field($id);
                }
            }
