...
class fieldtype_time
{
    ...
    else
    {
    	$html = '
                    <div class="fieldtype-time-input">        
                        ...
                        ' . input_tag('fields[' . $field['id'] . ']_hours',$hours,['class'=>'form-control field_hm_' .  $field['id'],'placeholder'=>'00','maxlength'=>3]) . '
                            <span class="input-group-addon" style="padding-left:0; padding-right: 0">:</span>
                        ' . input_tag('fields[' . $field['id'] . ']_minutes',$minutes,['class'=>'form-control field_hm_' .  $field['id'],'placeholder'=>'00','maxlength'=>2]) . '                        
                        </div>    																		
                        ' . input_hidden_tag('fields[' . $field['id'] . ']',$value,['class'=>'field_' . $field['id'] . ' ' . ($field['is_required']==1 ? ' required':'')]) .'    
                    </div>        
  ...
  {
  	$history_query = db_query("select sum(fields_value+0) as total from app_comments_history where fields_id='" . $field_id. "' and comments_id in (select id from app_comments where entities_id='" . db_input($entity_id) . "' and items_id='" . db_input($item_id) . "')");
