            ...
            if(preg_match_all('/\[(\w+)\]/', $pattern, $matches))
            {
                ...
                foreach($matches[1] as $matches_key => $fields_id)
                {
                    ...
                    if($field)
                    {
                        ...
                        if(in_array($field['type'], array('fieldtype_textarea_wysiwyg','fieldtype_qrcode')))
                        {
                            $output = trim(fields_types::output($output_options));
                        }
                        elseif(in_array($field['type'], array('fieldtype_items_by_query')) and isset($options['is_email']))
                        {
                            $output = trim(fields_types::output($output_options));
                        }
                        ...
                        elseif(in_array($field['type'], array('fieldtype_image','fieldtype_image_ajax')))
                        {
                        ...
                        elseif(!isset($output_options['is_export']) and in_array($field['type'], array('fieldtype_attachments', 'fieldtype_input_file', 'fieldtype_image','fieldtype_onlyoffice')))
                        {
                            $output = fields_types::output($output_options);
                        }
                        elseif( in_array($field['type'], array('fieldtype_todo_list')))
                        {
                            $output = trim(fields_types::output($output_options));
                        }
                        else
