...
class fieldtype_items_by_query
{
    ...
    function get_configuration()
    {
        ...
        $cfg[TEXT_DISPLAY_AS][] = array('title' => TEXT_HIDE_FIELD_IF_EMPTY, 'name' => 'hide_field_if_empty_result', 'type' => 'checkbox', 'tooltip_icon' => TEXT_HIDE_FIELD_IF_EMPTY_TIP);
    ...
    function output($options)
    {
        ...
        if($count_items==0 and $cfg->get('hide_field_if_empty_result')==1 and ($options['is_item_page']??0)==1 and !strlen($debug_query))
        {
            $debug_query = '<style>.form-group-' . $options['field']['id'] . '{ display:none }</style>';
        }
