...
class fieldtype_years_difference
{
	...
	function get_configuration()
	{
		...
		$fields_query = db_query("select * from app_fields where type in ('fieldtype_input_date','fieldtype_input_datetime','fieldtype_dynamic_date','fieldtype_input_date_extra') and entities_id='" . db_input($_POST['entities_id']) . "'");
...
END;";
	...
	public static function prepare_query($fields,$prefix = 'e', $single_select = false, $force_query = false)
	{
		...
		//skip dynamic query
		if(isset($cfg->cfg['dinamic_query']) and $cfg->get('dinamic_query')!=1 and !$force_query) return $prefix . '.field_' . $fields['id'];
