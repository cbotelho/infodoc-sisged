...
class fieldtype_digital_signature
{
    ...
    function get_configuration()
    {
        ...
        $fields_query = db_query("select f.id,f.name, e.name as entity_name from app_fields f, app_entities e where e.id='" . $_POST['entities_id']. "' and e.id=f.entities_id and type in ('fieldtype_users','fieldtype_users_ajax')  order by e.sort_order, e.name, f.name");
        while($fields = db_fetch_array($fields_query))
        {
        ...
        if(is_ext_installed())
        {
            $processes_query = db_query("select id, name from app_ext_processes where entities_id='" . $_POST['entities_id'] . "' order by sort_order, name");
                ...
                if($fields['type']=='fieldtype_digital_signature')
                {
                    ...
                    if($item_info = db_fetch_array($item_info_query))
                    {
                        foreach($cfg->get('signature_fields') as $field_id)
                        {
                            ...
                            {
                                $signed_items_query = db_query("select id from  app_ext_signed_items where fields_id='" . $fields['id'] . "' and entities_id='" . $entities_id . "' and items_id='" . $items_id . "'");
                                while($signed_items = db_fetch_array($signed_items_query))
                                {
                                    db_query("delete from app_ext_signed_items where id='" . $signed_items['id'] . "'"); 
                                    db_query("delete from app_ext_signed_items_signatures where signed_items_id='" . $signed_items['id'] . "'");
                                }
                            }
                        }
                    }
