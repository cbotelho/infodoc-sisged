...
function db_query($query, $debug = false, $link = 'db_link')
{
    ...
    {
        $log = $e->getCode() . ' - ' . htmlspecialchars($e->getMessage()) . '<br><br>' . htmlspecialchars($query) . '<br><br>' . str_replace(DIR_FS_CATALOG,'',nl2br($e->getTraceAsString()));        
        app_logs::db_log($log, 0, 1);
        ...
        $html = '
            ...
            <div style="color: #b94a48; background: #f2dede; border: 1px solid #eed3d7; padding: 5px; margin: 5px; font-family: verdana; font-size: 12px; line-height: 1.5;">
              ...
              <div><strong>Query:</strong> ' . htmlspecialchars($query) . '</div>
              <div><strong>Page: </strong> ' . $_SERVER['REQUEST_URI'] . '</div>
              <div style="padding-top: 10px;">' . str_replace(DIR_FS_CATALOG,'',nl2br($e->getTraceAsString())) . '</div>
            </div>
        ';
        die($html);
    }
    ...
    if($debug)
    {        
        echo '<div class="alert alert-warning" style="font-size: 11px; margin: 5px; padding: 3px; font-family:monospace;">' . htmlspecialchars($query) . '<div style="padding-top: 5px;">[' . $time . ']</div></div><br>';
    }

    app_logs::db_log($query, $time);
