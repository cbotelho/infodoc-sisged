...
function build_user_menu()
{
    ...
    if(guest_login::is_guest())
    {
        ...
        {
            $menu[] = array('title' => TEXT_CHANGE_ACCESS_GROUP, 'url' => url_for('users/change_access_group'), 'modalbox' => true, 'class' => 'fa-user-o');
        }

        $menu[] = array('title' => TEXT_LOGOFF, 'url' => url_for('users/login&action=logoff'), 'class' => 'fa-sign-out');
    ...
    if(strlen($app_user['multiple_access_groups']))
    {
        $menu[] = array('title' => TEXT_CHANGE_ACCESS_GROUP, 'url' => url_for('users/change_access_group'), 'is_hr' => true, 'modalbox' => true, 'class' => 'fa-user-o');
    }
...
function build_custom_entities_menu($menu, $parent_id = 0, $level = 0)
{
    ...
    while($entities_menu = db_fetch_array($entities_menu_query))
    {
        ...
        //add urls
        if(in_array($entities_menu['type'], ['url', 'url_self']) and strlen($entities_menu['url']))
        {            
            if((strlen($entities_menu['users_groups']) and in_array($app_user['group_id'],explode(',',$entities_menu['users_groups']))) or strlen($entities_menu['assigned_to']) and in_array($app_user['id'],explode(',',$entities_menu['assigned_to'])))
            {
                ...
                $sub_menu[] = array(
                    ...
                    'class' => $menu_icon,
                    'target'=>$entities_menu['type']=='url_self' ? '_self' : '_blank',
        ...
        {                        
            $menu_icon = (strlen($entities_menu['icon']) > 0 ? $entities_menu['icon'] : (strlen($sub_menu[0]['class']) ? $sub_menu[0]['class'] : 'fa-reorder') );
            
            $menu[] = array(
                'title' => $entities_menu['name'], 
                'url' => $sub_menu[0]['url'], 
                'class' => $menu_icon,
                'icon_color' => strlen($entities_menu['icon_color']) ? $entities_menu['icon_color'] : ($sub_menu[0]['icon_color']??'') ,
                'bg_color' => $entities_menu['bg_color'],
...
function build_reports_groups_menu($menu)
{
    ...
    $reports_query = db_query("select * from app_reports_groups where (find_in_set(" . $app_user['group_id'] . ",users_groups) or find_in_set({$app_user['id']},assigned_to)) and is_common=1 and in_menu=1 order by sort_order, name");
    while($v = db_fetch_array($reports_query))
    {
        $check_query = db_query("select id from app_entities_menu where find_in_set('dashboard" . $v['id'] . "',reports_list)");
        if(!$check = db_fetch_array($check_query))
...
function has_access_to_call_history()
{
    global $app_user;
    
    if(CFG_CALL_HISTORY_IN_MNU==1 and strlen(CFG_CALL_HISTORY_ACCESS) and in_array($app_user['group_id'],explode(',',CFG_CALL_HISTORY_ACCESS)))
    {
       return true;
    }
    else
...
function build_call_history_menu($menu)
{
    global $app_user;
    
    if(has_access_to_call_history())
    {
        $menu[] = array('title' => TEXT_EXT_CALL_HISTORY, 'url' => url_for('ext/call_history/view'), 'class' => 'fa-phone');
    }
...
function build_main_menu()
{
    ...
    if($app_user['group_id'] == 0)
    {
        ...
        $ss[] = array('title' => TEXT_WHO_IS_ONLINE, 'url' => url_for('configuration/who_is_online'));
        ...
        $ss[] = array('title' => TEXT_LOGIN_ATTEMPT, 'url' => url_for('configuration/login_attempt'));
        ...
        if(CFG_WHO_IS_ONLINE_STATUS)
        {
            $s[] = array('title' => TEXT_WHO_IS_ONLINE, 'url' => url_for('tools/who_is_online'));
        }
...
function renderSidebarMenu($menu = array(), $html = '', $level = 0)
{
    ...
    foreach($menu as $v)
    {
        ...
        $icon_color = (isset($v['icon_color']) and strlen($v['icon_color'])) ? 'style="color: ' . $v['icon_color'] . '"':'';
...
function isSidebarMenuItemActive($menu, $menu_url, $menu_level, $check_level = 0)
{
    ...
    if((strstr($current_url, $menu_url) && strlen($current_url)==strlen($menu_url)) || strstr($current_url, $menu_url . '&'))        
    {        
        return true;
    }
    else
...
function hasSidebarLevelUrls($current_url, $url_list)
{
    foreach($url_list as $url)
    {
        if((strstr($current_url, $url) and strlen($current_url)==strlen($url)) || strstr($current_url, $url . '&'))        
        {
            return true;
        }
    }
...
function renderNavbarMenu($menu = array(), $html = '', $level = 0, $selected_id = 0)
{
    ...
    foreach($menu as $v)
    {
        ...
        if(isset($v['submenu']))
        {
            $html .= '<li class="dropdown ' . $menu_css . ' ' . (($selected_id == $v['selected_id'] or in_array($selected_id,$v['selected_ids']??[])) ? 'selected' : '') . '"><a href="#" class="dropdown-toggle ' . $menu_css . '" data-hover="dropdown" data-toggle="dropdown">' . $v['title'] . ' <i class="fa fa-angle-down"></i></a>';
        }
        else
        {
