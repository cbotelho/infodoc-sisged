...
function render_listing_search_form($entity_id, $listing_container, $reports_id, $input_width = 'input-medium')
{
  ...
  <form id="' . $listing_container . '_search_form" class="navbar-search listing-search-form" onSubmit="load_items_listing(\'' . $listing_container . '\',1); return false;">
    ...
    <script>
        $("#' . $listing_container . '_search_keywords").keyup(function(){
            if($(this).val().length>0)
            {
                $("#' . $listing_container . '_search_reset").val("")
            }
            else
            {
...
function app_send_new_comment_notification($comments_id, $item_id, $entity_id)
{
    ...
    if (CFG_EMAIL_COPY_SENDER == 1  and fieldtype_created_by::is_notification_enabled($entity_id))
    {
        $send_to[] = $app_user['id'];
    }
    else
    ...
    if (count($send_to) > 0)
    {
        ...
        $subject = (strlen($cfg->get('email_subject_new_comment')) > 0 ? $cfg->get('email_subject_new_comment') . ' ' . $item_name : TEXT_DEFAULT_EMAIL_SUBJECT_NEW_COMMENT . ' ' . $item_name);
        ...
        foreach (array_unique($send_to) as $user_id)
        {
            ...
            if (users_cfg::get_value_by_users_id($user_id, 'disable_notification') != 1 and $cfg->get('disable_notification')!=1)
            {
...
function i18n_js()
{
    ...
    $list['TEXT_NO_RESULTS_FOUND'] = TEXT_NO_RESULTS_FOUND;    
    $list['TEXT_MAXIMUM_NUMBER_OF_FIELS_TO_UPLOAD'] = TEXT_MAXIMUM_NUMBER_OF_FIELS_TO_UPLOAD;    
...
// add default logical operators if needed
    ...
    for ($i = 0; $i < count($objects); $i++)
    {
        ...
        elseif (isset($objects[$i]) && ($objects[$i] != '(') && ($objects[$i] != ')'))
        {
            $keyword_count++;
        }
    }
...
function app_render_icon($icon, $params = '', $color='')
{
    $icon = trim($icon??'');
...
function _GETS($v)
{
    if (isset($_GET[$v]))
    {
        return preg_replace('/[^\w_-]+/u','',$_GET[$v]);
    }
    else
    {
        die('Error: $_GET[' . $v . '] is not available!');
...
{
    if(!is_null(CFG_APP_COPYRIGHT_NAME) and strlen(CFG_APP_COPYRIGHT_NAME)>0)
    {
        if(strstr(CFG_APP_COPYRIGHT_NAME, '[year]'))
        {
            return '<span class="copyright-text">' . '&copy; ' . str_replace('[year]',  date('Y'),CFG_APP_COPYRIGHT_NAME) . '</span><br>';
        }
        else
        {
            return '<span class="copyright-text">' . '&copy; ' . CFG_APP_COPYRIGHT_NAME . ' ' . date('Y') . '</span><br>';
        }
    }
...
function fix_utc($val)
{
    $utc = date('P');
    $utc_operator = $utc[0];
    $utc_hours = (int)str_replace(['+','-'],'',explode(':',$utc)[0]);
    
    if($utc_operator=='+')
    {
        $val = strtotime("-{$utc_hours} hour",$val);
    }
    else
    {
        $val = strtotime("+{$utc_hours} hour",$val);
