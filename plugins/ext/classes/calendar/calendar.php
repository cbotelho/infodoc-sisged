...
class calendar
{
    ...
    {
        return is_file('js/fullcalendar-scheduler/6.1.10/packages/core/locales/' . APP_LANGUAGE_SHORT_CODE . '.global.js') ? APP_LANGUAGE_SHORT_CODE : 'en';
    }
    
    static public function getInitialView($mode)
    {
        switch($mode)
        {
            case 'year':
    ...
    static function get_view_modes($reports)
    {
        ...
        $modes = str_replace(['month','agendaWeek','agendaDay','listMonth'],['dayGridMonth','timeGridWeek','timeGridDay','listMonth'],$modes);
    ...
    static public function render_highlighting_weekends($highlighting_weekends)
    {
        if (strlen($highlighting_weekends??''))
        {
            return '
			<style>
				' . str_replace('.fc-','.fc-day-',$highlighting_weekends) . '{
					background-image: url(images/bg/gray-twill.png);
				}
				.fc-timeline-slot.fc-day{
    ...
    static public function get_css($reports)
    {
        ...
        while ($choices = db_fetch_array($choices_query))
        {
            ...
            $html .= '			
                    .fc-item-css-' . $choices['id'] . ':not(.fc-daygrid-dot-event) .fc-event-title {
    ...
    static function remove_reports_by_entity_id($entity_id)
    {
        $calendars_qeury = db_query("select id from app_ext_calendar where entities_id={$entity_id}");
        while($calendars = db_fetch_array($calendars_qeury))
        {
            self::remove_report_by_id($calendars['id']);
        }
    }
    
    static function remove_report_by_id($id)
    {
        db_delete_row('app_ext_calendar', $id);
        db_delete_row('app_ext_calendar_access', $id, 'calendar_id');

        $report_info_query = db_query("select * from app_reports where reports_type='calendarreport" . db_input($id) . "'");
