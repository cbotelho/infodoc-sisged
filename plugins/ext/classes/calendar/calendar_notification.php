...
{
	public $personal_calendar_send_alerts, $public_calendar_send_alerts;
	
  function __construct()
  {
  	$this->personal_calendar_send_alerts = (!defined('CFG_PERSONAL_CALENDAR_SEND_ALERTS') ? 0 : CFG_PERSONAL_CALENDAR_SEND_ALERTS);
  	$this->public_calendar_send_alerts = (!defined('CFG_PUBLIC_CALENDAR_SEND_ALERTS') ? 0 : CFG_PUBLIC_CALENDAR_SEND_ALERTS);
  }
  ...
  function send_personal()
  {
  	...
  	if(CFG_PERSONAL_CALENDAR_ALERTS_TIME!=date('H')) return false;                
  	
  	$users_query = db_query("select e.* from app_entity_1 e where e.field_5=1 and ((select count(*) as total from app_ext_calendar_access c where c.calendar_type='personal' and c.access_groups_id=e.field_6)>0 or e.field_6=0)");
  	while($user = db_fetch_array($users_query))
  	{
  		...
  		if(($events_count = count($events))>0)
  		{
  			...
  			$subject = (strlen(CFG_PERSONAL_CALENDAR_ALERTS_SUBJECT) ? CFG_PERSONAL_CALENDAR_ALERTS_SUBJECT : TEXT_EXT_PERSONAL_CALENDAR_ALERTS_SUBJECT);
  ...
  function render_events_html($events)
  {
  	...
  	foreach($events as $event)
  	{
  		...
  		//prepare event time
  		$when_html = '';
  		if($event['start_date']!=$event['end_date'] and date('Y-m-d',$event['start_date'])==date('Y-m-d',$event['end_date']))
  		{
  			$when_html = ' (' . sprintf(TEXT_FROM_TO,date('H:i',$event['start_date']),date('H:i',$event['end_date'])) . ')';
  		}
  		  		  		 
  		//prepare ongoing events
  		if($event['start_date']!=$event['end_date'] and date('Y-m-d',$event['start_date'])!=date('Y-m-d',$event['end_date']))
  		{
  			$when_html = ' (' . sprintf(TEXT_FROM_TO,format_date($event['start_date']),format_date($event['end_date'])) . ')';
  ...
  function send_public()
  {
  	...
  	//check notification time
  	if(CFG_PUBLIC_CALENDAR_ALERTS_TIME!=date('H')) return false;
  	 
  	$users_query = db_query("select e.* from app_entity_1 e where e.field_5=1 and ((select count(*) as total from app_ext_calendar_access c where c.calendar_type='public' and c.access_groups_id=e.field_6)>0 or e.field_6=0)");
  	while($user = db_fetch_array($users_query))
  	{
  		$app_user = array('id'=>$user['id'],
  				'group_id'=>(int)$user['field_6'],
  		...
  		if(($events_count = count($events))>0)
  		{
  			...
  			$subject = (strlen(CFG_PUBLIC_CALENDAR_ALERTS_SUBJECT) ? CFG_PUBLIC_CALENDAR_ALERTS_SUBJECT : TEXT_EXT_PUBLIC_CALENDAR_ALERTS_SUBJECT);
