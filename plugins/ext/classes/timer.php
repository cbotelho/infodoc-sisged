...
class timer{
  ...
  static function render_header_dropdown_menu()
  {
    ...
    $count_timers_query = db_query("select count(*) as total from app_ext_timer where  users_id='" . db_input($app_user['id']) . "'");
    ...
    if($count_timers['total']>0)
    {
      ...
      while($timers = db_fetch_array($timers_query))
      {
        ...
        $item_info_query = db_query("select e.* from app_entity_" . $timers['entities_id'] . " e where id='" . $timers['items_id'] . "'");
