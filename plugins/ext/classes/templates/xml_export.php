...
class xml_export
{
    ...
    function prepare_related_entities($xml, $templates,  $field_id, $entity_id, $related_items)
    {
        ...
        $query = new items_query($entity_id,[ 
                ...
                'add_formula' => true,
                'add_filters' => true,
                'report_id'=> reports::get_reports_id_by_type($entity_id, 'xml_export' . $templates['id'] . '_field_' . $field_id)
            ]);
    ...
    function prepare_nested_entities($xml, $templates,  $entity_id, $parent_item_id)
    {
        ...
        $query = new items_query($entity_id,[ 
                ...
                'add_formula' => true,    
                'add_filters' => true,
                'report_id'=> reports::get_reports_id_by_type($entity_id, 'xml_export' . $templates['id'] . '_entity_' . $entity_id)
            ]);
    ...
    static function get_linked_entities_filters_html($template)
    {
        global $app_entities_cache;
        
        if(strlen($template['related_entities_template']))
        {
            $related_entities_template = json_decode($template['related_entities_template'],true);
        }
        else
        {
            $related_entities_template = [];
        }        
        
        $filters_list = [];
        $entities_query = db_query("select * from app_entities where parent_id = '" . $template['entities_id'] . "'");
        while($entities = db_fetch_array($entities_query))
        {
            if(!strlen($related_entities_template['entity_' . $entities['id']]??''))
            {
                continue;
            }
            
            $filters_list[] = [
                'type' => 'xml_export' . $template['id'] . '_entity_' . $entities['id'],
                'entity_id' => $entities['id'],
                'name' => $entities['name']
            ];
        }
        
        $fields_query = db_query("select id, name, configuration, entities_id,type from app_fields where entities_id='" . $template['entities_id'] . "' and type in ('fieldtype_entity','fieldtype_entity_ajax','fieldtype_entity_multilevel','fieldtype_related_records','fieldtype_users','fieldtype_users_ajax')");
        while($fields = db_fetch_array($fields_query))
        {
            if(!strlen($related_entities_template['field_' . $fields['id']]??''))
            {
                continue;
            }
            ...
            $filters_list[] = [
                'type' => 'xml_export' . $template['id'] . '_field_' . $fields['id'],
                'entity_id' => $entity_id,
                'name' => $fields['name'] . ': ' . $entity_name
            ];
        }
        ...
        {           
            ...
            $html = [];
            foreach($filters_list as $v)
            {
                $report_id = reports::auto_create_report_by_type($v['entity_id'],$v['type']);                
                $count_filters = reports::count_filters_by_reports_id($report_id);
                $html[] = link_to($v['name'] . ' (' . $count_filters . ')',url_for('default_filters/filters','reports_id=' . $report_id . '&redirect_to=xml_export' . $template['id']));
            }
            return TEXT_EXT_FILTERS_FOR_LINKED_ENTITIES . ': ' . implode(' | ', $html);
        }
    }
