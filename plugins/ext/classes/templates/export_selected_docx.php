...
{
    public $template_info, $items_list, $templateProcessor, $selected_items, $entities_id;
    ...
    function prepare_template_table_blocks()
    {
        ...
        $listing_sql_query_select = fieldtype_formula::prepare_query_select($this->entities_id, '', false, array('fields_in_listing' => implode(',',$fields_in_listing)));
        ...
        foreach($items_query as $item_count=>$item)
        {
            ...
            $blocks_query = db_query("select b.*,f.is_heading, f.name, f.entities_id, f.type as field_type,f.configuration as field_configuration from app_ext_export_selected_blocks b, app_fields f, app_entities e where  block_type='body_cell' and b.fields_id=f.id and b.templates_id = " . $this->template_info['id'] . " and b.parent_id = " . $parent_block_id . " and f.entities_id=e.id order by b.sort_order, b.id",false);
            while($blocks = db_fetch_array($blocks_query))
            {
        ...
        $this->templateProcessor->setValue('${count}',(string)$number_of_rows);                
    ...
    function prepare_extra_rows($wordTable,$parent_block_id,$block_type,$header_bg_color = '',$parent_block_settings='')
    {
        ...
        $rows_query = db_query("select b.* from app_ext_export_selected_blocks b where b.block_type='" . $block_type . "' and b.templates_id = " . $this->template_info['id'] . " and b.parent_id = " . $parent_block_id . " order by b.sort_order, b.id");                        
        while($rows = db_fetch_array($rows_query))
        {                                              
            $blocks_query = db_query("select b.* from app_ext_export_selected_blocks b where b.templates_id = " . $this->template_info['id'] . " and b.parent_id = " . $rows['id'] . " order by b.sort_order, b.id");
