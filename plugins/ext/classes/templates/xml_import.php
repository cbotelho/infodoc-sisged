...
class xml_import
{
    ...
    function get_file_by_path()
    {
        ...
        self::prepare_file_content($this->filename);
    }    
    
    //prepare file content before load
    static function prepare_file_content($file)
    {        
        $xml = file_get_contents(DIR_FS_TMP . $file);        
        $xml = preg_replace('/"urn:[^"]+"/','""',$xml);
        file_put_contents(DIR_FS_TMP . $file,$xml);
    }
