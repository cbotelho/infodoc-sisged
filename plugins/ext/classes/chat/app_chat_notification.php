...
class app_chat_notification
{
    ...
    function __construct()
    {
        ...
        $this->sound_file = (strlen($app_users_cfg->get('chat_sound_notification')) ? $app_users_cfg->get('chat_sound_notification') : CFG_CHAT_SOUND_NOTIFICATION);
        
        $this->has_sound_notification = (strlen($this->sound_file) ? true:false); 
        
        if($app_users_cfg->get('chat_instant_notification')==1 or (CFG_CHAT_INSTANT_NOTIFICATION==1 and $app_users_cfg->get('chat_instant_notification')!=0))
        {
    ...
    function send($current_unread_all_msg)
    {
        ...
        {                                    
            ...
            {
                $body = '';
                $message_query = db_query("select * from app_ext_chat_messages where assigned_to='" . $app_user['id']. "' order by id desc limit 1");
                if($message = db_fetch_array($message_query))
                {                
                    $body .=  addslashes(strip_tags($app_users_cache[$message['users_id']]['name'])) . '\n' . addslashes(strip_tags($message['message']));
                }
                ...
                $html .= '
                    <script>                      
                        if (("Notification" in window) && (!is_app_caht_timer || document.hidden)) 
                        {
                            ...
                            {
                                var notification = new Notification("' . sprintf(TEXT_EXT_NEW_MSG_IN_CHAT,$current_unread_all_msg). '",{
                                    dir: app_language_text_direction,
                                    body: "' . $body . '",
                                    sound: "' . (strlen($this->sound_file) ? 'js/ion.sound-master/sounds/' . $this->sound_file . '.mp3':''). '",
                                    });
