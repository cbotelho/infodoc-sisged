...
class item_pivot_tables
{
    ...
    public $position, $entities_id, $reports, $number_of_rows_per_page, $current_page_number, $number_of_rows, $number_of_pages;
    ...
    function render()
    {
        ...
        $reports_query = db_query("select * from app_ext_item_pivot_tables where entities_id='" . $this->entities_id . "' and position='" . $this->position . "' and find_in_set(" . $app_user['group_id'] . ",allowed_groups)  order by sort_order, name");
				...
				<script>
				   function load_item_pivot_tables(reports_id,page)
				   {
    				 ...
    				 $("#item_pivot_tables_"+reports_id).css("opacity", 0.5);
				   	 ...
				   	 $("#item_pivot_tables_"+reports_id).load("' . url_for('items/item_pivot_tables_report', 'path=' . $app_path) . '&reports_id="+reports_id+"&page="+page,function(){
    ...
    function render_report()
    {
        ...
        $fields_query = db_query("select f.* from app_fields f where f.id in (" . $this->reports['related_entities_fields'] . ") order by field(f.id," . $this->reports['related_entities_fields'] . ")");
        while($fields = db_fetch_array($fields_query))
        {
            ...
            switch($fields['type'])
            {
                ...
                case 'fieldtype_entity_ajax':
                    ...
                    $default_reports_query = db_query("select * from app_reports where entities_id='" . db_input($cfg->get('entity_id')) . "' and reports_type='item_pivot_tables_" . $this->reports['id'] . "_" . $fields['id'] . "'");
                    ...
                    while($item = db_fetch_array($items_query))
                    {
                        ...
                        {
                            $fields_choices[$item['id']] = '<a href="' . url_for('items/info', 'path=' . $cfg->get('entity_id') . '-' . $item['id']) . '">' . items::get_heading_field($cfg->get('entity_id'), $item['id'], $item) . '</a>';
                        }
                        else
                        {
                            $fields_choices[$item['id']] = items::get_heading_field($cfg->get('entity_id'), $item['id'], $item);
                        }
                    }

                    $type = 'item_pivot_tables_' . $this->reports['id'] . "_" . $fields['id'];
                    $filters_panels = new filters_panels($default_reports['entities_id'], $default_reports['id'], $this->reports['id'], 0);
                    $filters_panels->set_type($type);
                    $filters_panels->set_items_listing_funciton_name('load_item_pivot_tables');
                    $filters_panels_html .= '<div class="' . $type . '">' . $filters_panels->render_horizontal() . '</div>';
        ...
        foreach($fields_id_list as $field_id)
        {
            if($level == 1)
            {
                ...
                {
                    $level_choices[$level][] = [['fileds_id' => $field_id, 'type' => $fields_list[$field_id]['type'], 'cfg' => $fields_list[$field_id]['cfg'], 'choices_id' => $choices_id, 'choices_value' => $value]];
                }
            }
            else
            {
                ...
                foreach($level_choices[$level - 1] as $previous_value)
                {
                    foreach($fields_list[$field_id]['choices'] as $choices_id => $value)
                    {
                        $level_choices[$level][] = array_merge($previous_value, [['fileds_id' => $field_id, 'type' => $fields_list[$field_id]['type'], 'cfg' => $fields_list[$field_id]['cfg'], 'choices_id' => $choices_id, 'choices_value' => $value]]);
                    }
                }
            }
                    ...
                    if(strlen($this->reports['fields_in_listing']))
                    {
                        ...
                        if(isset($fields_in_listing[$cfg->get('entity_id')]))
                        {
                            foreach($fields_in_listing[$cfg->get('entity_id')] as $fields_id)
                            {
                                ...
                                {
                                    $html .= '<th>' . fields_types::get_option($field_info['type'], 'name', $field_info['name']) . '</th>';
                                }
                            }
                        }
                    }
        ...
        $columns_query = db_query("select * from app_ext_item_pivot_tables_calcs where type='column' and reports_id='" . $this->reports['id'] . "' order by sort_order, name");
    ...
    function display_page_number_links()
    {
        ...
        if($cur_window_num > 1)
            $html .= '<li><a href="#" onClick="load_item_pivot_tables(' . $this->reports['id'] . ',' . (($cur_window_num - 1) * $max_page_links) . '); return false;" title=" ' . sprintf(PREVNEXT_TITLE_PREV_SET_OF_NO_PAGE, $max_page_links) . ' ">...</a></li>';
        ...
        for($jump_to_page = 1 + (($cur_window_num - 1) * $max_page_links); ($jump_to_page <= ($cur_window_num * $max_page_links)) && ($jump_to_page <= $this->number_of_pages); $jump_to_page++)
        {
            ...
            {
                $html .= '<li><a href="#" onClick="load_item_pivot_tables(' . $this->reports['id'] . ',' . $jump_to_page . '); return false;" title=" ' . sprintf(PREVNEXT_TITLE_PAGE_NO, $jump_to_page) . ' ">' . $jump_to_page . '</a></li>';
            }
        }

        // next window of pages
        if($cur_window_num < $max_window_num)
            $html .= '<li><a href="#"  onClick="load_item_pivot_tables(' . $this->reports['id'] . ',' . ($cur_window_num * $max_page_links + 1) . ')" title=" ' . sprintf(PREVNEXT_TITLE_NEXT_SET_OF_NO_PAGE, $max_page_links) . ' ">...</a></li>';
                ...
                switch($choices['type'])
                {
                    ...
                    case 'fieldtype_entity_ajax':
                        if(strlen($this->reports['fields_in_listing']))
                        {
                            ...
                            if(isset($fields_in_listing[$cfg->get('entity_id')]))
                            {
                                $item_query = db_query("select e.* from app_entity_" . $cfg->get('entity_id') . " e where e.id='" . $choices['choices_id'] . "'");
                                if($item = db_fetch_array($item_query))
                                {
                                    ...
                                    foreach($fields_in_listing[$cfg->get('entity_id')] as $fields_id)
                                    {
                                        $field_query = db_query("select id, type, name, configuration, entities_id from app_fields where id='" . $fields_id . "'");
    ...
    function build_calculations($row)
    {
        ...
        $calculations_query = db_query("select * from app_ext_item_pivot_tables_calcs where type='calc' and reports_id='" . $this->reports['id'] . "' order by  name");
        while($calculations = db_fetch_array($calculations_query))
        {
            ...
            if(strlen($calculations['where_query']))
            {
                ...
                foreach($where_query as $entity_id => $entity_where_query)
                {
                    ...
                    if($count == 0)
                    {
                        $mysql_query .= "select (" . $calculations['select_query'] . ") as total from app_entity_" . $entity_id . " {$prefix} where {$prefix}.id>0 " . (strlen($entity_where_query) ? ' and ' . $entity_where_query : '');
                    ...
                    else
                    {
                        ...
                        $mysql_query .= " and {$prefix2}.parent_item_id in (select {$prefix}.id from app_entity_" . $entity_id . " {$prefix} where {$prefix}.id>0  " . (strlen($entity_where_query) ? ' and ' . $entity_where_query : '');
                    }
            ...
            foreach($row as $choices)
            {
                $mysql_query = str_replace('[field_' . $choices['fileds_id'] . '_value]', $choices['choices_id'], $mysql_query);
            }
