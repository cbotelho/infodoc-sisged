...
class mind_map_reports
{
	...
	function __construct($reports, $path)
	{
		...
		$this->icons = strlen($reports['icons']) ? json_decode($reports['icons'],true) : array();
		$this->path = $path;
	...
	function set_background_colors()
	{
		...
		{
			$choices_query = db_query("select * from app_fields_choices where fields_id = '" . db_input($this->use_background). "' and length(bg_color)>0");
		}
	...
	function render_legend()
	{
		...
		{	
			...
			if($field_info = db_fetch_array($field_info_query))
			{
				...
				{
					$choices_query = db_query("select * from app_fields_choices where fields_id = '" . db_input($this->use_background). "'");
				}
				...
				while($choices = db_fetch_array($choices_query))
				{
					$html .= '
							<tr>
								<td style="width: 35px; ' . (strlen($choices['bg_color']) ? 'color: ' . $choices['bg_color'] : ''). '"><i class="fa ' . (strlen($this->icons[$choices['id']]) ? $this->icons[$choices['id']] : 'fa-angle-down') . '"></i></td>
								<td style="text-align: left; font-size: 11px; ' . (strlen($choices['bg_color']) ? 'color: ' . $choices['bg_color'] : ''). '">' . $choices['name'] . '</td>
							</tr>
							';
					//$this->background_colors[$choices['id']] = $choices['bg_color'];
				}
	...
	function prepare_deleted_items($data)
	{
		...
		$mm_query = db_query("select id, mm_items_id from app_mind_map where " . (count($chilider_list) ? " mm_id not in (" . implode(',',$chilider_list). ") and  length(mm_parent_id)>0 and " : '') . " entities_id='" . $this->entities_id . "' and reports_id='" . $this->reports_id . "' and parent_entity_item_id = '" . $this->parent_entity_item_id . "'");
	...
	function save_root($data)
	{
		...
		$mm_query = db_query("select id from app_mind_map where mm_id='" . $data['id']. "' and entities_id='" . $this->entities_id . "' and reports_id='" . $this->reports_id . "' and parent_entity_item_id = '" . $this->parent_entity_item_id . "'");
	...
	function save_children($data)
	{
		if(isset($data['children']))
		{
			...
			foreach($data['children'] as $children)
			{
				...
				$mm_query = db_query("select id,mm_items_id from app_mind_map where mm_id='" . $children['id']. "' and entities_id='" . $this->entities_id . "' and reports_id='" . $this->reports_id . "' and parent_entity_item_id = '" . $this->parent_entity_item_id . "'");
				if($mm = db_fetch_array($mm_query))
				{
					db_perform('app_mind_map', $sql_data,'update',"id='" . $mm['id'] . "'");
	...
	function update_item($item_id, $name)
	{
		...
		if($heading_field_id)
			db_query("update app_entity_" . $this->entities_id . " set field_" . $heading_field_id . " = '" . db_input(strip_tags($name)) . "' where id='" . $item_id . "'");
	...
	function save_item($mind_map_id, $name)
	{
		...
		$fields_query = db_query("select f.* from app_fields f where f.type not in (" . fields_types::get_reserverd_types_list(). ",'fieldtype_related_records') and  f.entities_id='" . db_input($this->entities_id) . "' order by f.sort_order, f.name");
	...
	{
		//prepare root
		$mm_query = db_query("select * from app_mind_map where mm_parent_id='' and entities_id='" . $this->entities_id . "' and reports_id='" . $this->reports_id . "' and parent_entity_item_id = '" . $this->parent_entity_item_id . "'");
		if(!$mm = db_fetch_array($mm_query))
		{
				$sql_data = array(
						'mm_id' => 'root',
		...
		//prepare new items
		$itesm_query = db_query("select e.* from app_entity_" . $this->entities_id . " e where e.id not in (select mm.mm_items_id from app_mind_map mm where mm.entities_id='" . $this->entities_id . "' and mm.reports_id='" . $this->reports_id . "' and parent_entity_item_id = '" . $this->parent_entity_item_id . "') " . ($this->parent_entity_item_id ? " and e.parent_item_id='" . $this->parent_entity_item_id . "'":""));
		while($itesm = db_fetch_array($itesm_query))
		{
			$sort_order_query = db_query("select max(sort_order) as max_sort_order from app_mind_map where mm_parent_id='root' and entities_id='" . $this->entities_id . "' and reports_id='" . $this->reports_id . "' and parent_entity_item_id = '" . $this->parent_entity_item_id . "'");
	...
	function get_tree($data = array(), $mm_parent_id='')
	{
		...
		$mm_query = db_query("select * from app_mind_map where mm_parent_id='" . $mm_parent_id. "' and entities_id='" . $this->entities_id . "' and reports_id='" . $this->reports_id . "' and parent_entity_item_id = '" . $this->parent_entity_item_id . "' order by sort_order");
		while($mm = db_fetch_array($mm_query))
		{						
			if(strlen($mm_parent_id))
			{
				...
				$check_query = db_query("select id from app_mind_map where mm_parent_id='" . $mm['mm_id']. "' and entities_id='" . $this->entities_id . "' and reports_id='" . $this->reports_id . "' and parent_entity_item_id = '" . $this->parent_entity_item_id . "' limit 1");
				if($check = db_fetch_array($check_query))
				{
	...
	function get_popup($items)
	{
		...
		if(strlen($this->fields_in_popup))
		{
			...
			foreach(explode(',',$this->fields_in_popup) as $fields_id)
			{
				...
				if($field = db_fetch_array($field_query))
				{
					...
					if(strlen(strip_tags($value))>255 and in_array($field['type'],['fieldtype_textarea_wysiwyg','fieldtype_textarea'])) $value = htmlspecialchars(substr(strip_tags($value),0,255)) . '...';
		...
		return addslashes(str_replace(array("'",'"'),array('&apos;','&quot;'),str_replace(array("\n","\r","\n\r","\t"),'',$html)));
	}
	...
	{
		$mm_query = db_query("select * from app_mind_map where entities_id='" . $entities_id . "' and mm_items_id='" . $items_id . "'");
		if($mm = db_fetch_array($mm_query))
		{
			$mm_parent_query = db_query("select * from app_mind_map where entities_id='" . $entities_id . "' and mm_parent_id='" . $mm['mm_id'] . "'");
			if($mm_parent = db_fetch_array($mm_parent_query))
			{
				db_query("update app_mind_map set mm_parent_id='" . $mm['mm_parent_id'] . "' where id='" . $mm_parent['id'] . "'");
			}
