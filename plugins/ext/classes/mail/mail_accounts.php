...
class mail_accounts
{
    ...
    static function get_signature_choices()
    {
        global $app_user;

        $choices = [];
        
        $accounts_query = db_query("select ma.*, mau.signature from app_ext_mail_accounts ma, app_ext_mail_accounts_users mau where ma.is_active=1 and ma.id=mau.accounts_id and mau.users_id='" . $app_user['id'] . "'");
                        
        while($accounts = db_fetch_array($accounts_query))
        {
            $choices[$accounts['id']] = $accounts['signature'];          
        }
    ...
    static function render_menu_item($menu)
    {
        if(CFG_MAIL_INTEGRATION == 1)
        {
            if(CFG_MAIL_DISPLAY_IN_MENU == 1 and mail_accounts_users::has_access())
            {
                ...
                $menu[] = array('title' => TEXT_EXT_INBOX, 'url' => url_for('ext/mail/accounts', 'action=set_folder&folder=inbox'), 'class' => 'fa-envelope', 'badge' => 'badge-info mail-menu-inbox-count ' . ($count == 0 ? 'hidden' : ''), 'badge_content' => ($count < 100 ? $count : '99+') );
            }
        }
    ...
    static function get_folders_choices()
    {
        ...
        $choices['unread'] = TEXT_EXT_UNREAD;
