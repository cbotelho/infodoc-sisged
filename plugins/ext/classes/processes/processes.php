...
class processes
{
    ...
    static function run_by_id($process_id, $item_id)
    {
        $process_query = db_query("SELECT * FROM app_ext_processes WHERE id ='{$process_id}'"); 
        if ($process = db_fetch_array($process_query)) 
        {   
            $item_query = db_query("select id from app_entity_{$process['entities_id']} where id='{$item_id}'");
    ...
    function run_after_move($items_id)
    {
        $this->items_id = $items_id;

        foreach($this->get_buttons_list('run_after_move') as $process_info)
        {
            if($this->check_buttons_filters($process_info))
            {
                $_post_fields = $_POST['fields'] ?? []; //save post fields
        ...
        switch($position)
        {
            ...
            case 'default':
                ...
                foreach($buttons_list as $buttons)
                {
                    ...
                    {
                        $html .= '<li>' . button_tag($buttons['button_title'], url_for('items/processes_warning', 'id=' . $buttons['id'] . '&path=' . $app_path), true, array('class' => 'btn btn-primary btn-sm btn-process-' . $buttons['id'] ), $buttons['button_icon']) . '</li>';
                        $html .= $this->prepare_button_css($buttons);
                    }
                }
            ...
            case 'comments_section':
                ...
                foreach($buttons_list as $buttons)
                {
                    ...
                    elseif($this->button_has_warnign_text($buttons))
                    {
                        $html .= button_tag($buttons['button_title'], url_for('items/processes_warning', 'id=' . $buttons['id'] . '&path=' . $app_path), true, array('class' => 'btn btn-primary btn-sm btn-process-' . $buttons['id']), $buttons['button_icon']);
                        $html .= $this->prepare_button_css($buttons);
                    }
                }
                break;
    ...
    {
        $check_query = db_query("select count(*) as total from app_ext_processes_actions_fields af where af.enter_manually in (1,2,3) and af.actions_id in (select pa.id from app_ext_processes_actions pa where pa.process_id='" . $process_id . "')");
    ...
    {
        $check_qeury = db_query("select count(*) as total  from app_ext_processes_actions where process_id='" . $process_id . "' and locate('move_item_entity_',type)>0 and is_active=1");
        $check = db_fetch_array($check_qeury);

        return ($check['total'] > 0 ? true : false);
    }
    
    public function has_insert_item_entity_action($process_id)
    {
        $check_qeury = db_query("select id from app_ext_processes_actions where process_id='" . $process_id . "' and locate('insert_item_entity_',type)>0 and is_active=1");
    ...
    public function has_clone_action_to_nested_entity($process_id)
    {
        ...
        $actions_qeury = db_query("select settings  from app_ext_processes_actions where process_id='" . $process_id . "' and locate('clone_item_entity_',type)>0 and is_active=1");
    ...
    public function has_copy_action($process_id, $check_parent = true)
    {
        ...
        $check_qeury = db_query("select count(*) as total  from app_ext_processes_actions where process_id='" . $process_id . "' and locate('copy_item_entity_',type)>0 and is_active=1");
        $check = db_fetch_array($check_qeury);
    ...
    public function get_buttons_list($position = '', $buttons_id = '')
    {
        ...
        while($buttons = db_fetch_array($buttons_query))
        {
            ...
            //check assigned to all
            if($buttons['assigned_to_all']==1)
            {
                $has_access = true;
            }
    ...
    public function check_buttons_filters($buttons)
    {
        ...
        $reports_info_query = db_query("select r.id, (select group_concat(rf.fields_id) from app_reports_filters rf where rf.reports_id=r.id) as filter_fields from app_reports r  where r.entities_id='" . db_input($buttons['entities_id']) . "' and r.reports_type='process" . $buttons['id'] . "'");
        if($reports_info = db_fetch_array($reports_info_query))
        {
            ...
            $listing_sql_query_select = fieldtype_formula::prepare_query_select($this->entities_id, $listing_sql_query_select,false,['fields_in_query'=>$reports_info['filter_fields']]);
            ...
            $item_info_query = db_query($item_info_sql,false);
            if($item_info = db_fetch_array($item_info_query))
            {
                if(is_null($item_info['id']))
                {
                   return false;
                }
                else
    ...
    public function preapre_values_from_current_item($sql_data, $process_info,$action_id, $item_id)
    {
        ...
        {
            if(isset($sql_data_holder[$process_info['id']][$action_id][$k]))
            {
                $v = $sql_data_holder[$process_info['id']][$action_id][$k];
            }
            ...
            if(preg_match('/\[\d+\]/', $v) or strstr($v, '[created_by]') or strstr($v, '[current_user_id]') or strstr($v, '[current_user_merge_value]') or strstr($v, '[current_user_exclude_value]'))
            {
                $check = true;
            }
        }

        //echo '<p>' . $item_id . ' = ' . $check . '</p>';
        //print_rr($sql_data);
        //print_rr($sql_data_holder);        
        //exit();
        ...
        if($check)
        {
            ...
            foreach($sql_data as $k => $v)
            {
                ...
                if(!isset($sql_data_holder[$process_info['id']][$action_id][$k]))
                {
                    $sql_data_holder[$process_info['id']][$action_id][$k] = $v;
                }
                else
                {
                    $v = $sql_data_holder[$process_info['id']][$action_id][$k];
                ...
                //use current user ID
                if(strstr($v, '[current_user_merge_value]'))
                {
                    $v = trim(str_replace('[current_user_merge_value]', $app_user['id'], $v));
                    $v = implode(',', array_unique(array_merge(explode(',',$item_info[$k]??''),[$v])));
                    $sql_data[$k] = $v;
                }
                
                if(strstr($v, '[current_user_exclude_value]'))
                {
                    $v = trim(str_replace('[current_user_exclude_value]', $app_user['id'], $v));
                    $v = implode(',', array_unique(array_diff(explode(',',$item_info[$k]??''),[$v])));
                    $sql_data[$k] = $v;
                }
            }
        }
    ...
    {
        global $app_path, $app_redirect_to, $app_user, $app_selected_items, $alerts, $sql_data_holder, $item_info_holder, $current_item_id, $app_entities_cache, $app_fields_cache, $app_force_print_template;
        ...
        elseif($has_insert_item_entity_action = $this->has_insert_item_entity_action($process_info['id']))
        {
            $selected_items = array(0);

            $actions_query_sql = " and locate('insert_item_entity_',type) ";
        }
        else
        {
        ...
        $actions_query = db_query("select pa.*, p.name as process_name, p.entities_id from app_ext_processes_actions pa, app_ext_processes p where pa.is_active=1 and  pa.process_id='" . $process_info['id'] . "' {$actions_query_sql} and  p.id=pa.process_id order by pa.sort_order",false);
        while($actions = db_fetch_array($actions_query))
        {
            ...
            //hold sql data for each process
            if(!isset($sql_data_holder[$process_info['id']][$actions['id']]))
            {
                $sql_data_holder[$process_info['id']][$actions['id']] = array();
            }
            ...
            while($actions_fields = db_fetch_array($actions_fields_query))
            {
                ...
                if(isset($fields_access_schema[$actions_fields['fields_id']]) and $process_info['apply_fields_access_rules'] == 1 and in_array($actions_fields['enter_manually'], [1, 2]))
                    continue;
                ...
                else
                {
                    ...
                    elseif($actions_fields['type'] == 'fieldtype_input_datetime' or $actions_fields['type'] == 'fieldtype_input_date_extra')
                    {
                        $actions_fields['value'] = ($actions_fields['value'] == ' ' ? 0 : (strlen($actions_fields['value']) < 5 ? get_date_timestamp(date('Y-m-d H:i',strtotime($actions_fields['value'] . ' day'))) : $actions_fields['value']));
                    }                    
                }
                ...
                switch($actions_fields['type'])
                {
                    case 'fieldtype_users_approve':
                        ...
                        {
                            $actions_fields['value'] = str_replace('[current_user_id]',$app_user['id'],$actions_fields['value']);
            ...
            if(count($sql_data) or strstr($actions['type'], 'edit_item_entity_') or
                    ...
                    strstr($actions['type'], 'runphp_item_entity_') or
                    strstr($actions['type'], 'delete_item_entity_')                    
            )                    
            {
                foreach($selected_items as $item_id)
                {
                    //echo '<pre>';
                    //echo 'item=' . $item_id . ' - acton id = ' . $actions['id'] . '<br>';
                    //handle values from current item
                    $sql_data = $this->preapre_values_from_current_item($sql_data, $process_info,$actions['id'], $item_id);
                    ...
                    switch(true)
                    {
                        ...
                        case strstr($actions['type'], 'edit_item_entity_'):
                            ...
                            //disable comments
                            if(($process_info['disable_comments'] == 1 and !strlen($description)) or $action_entity_cfg->get('use_comments') != 1)
                                $has_comment = false;
                        ...
                        case strstr($actions['type'], 'delete_item_entity_'):
                                                        
                            $path_info = items::get_path_info($action_entity_id, $item_id);
                            $items_to_delete = items::get_items_to_delete($action_entity_id, [$action_entity_id => $selected_items]);

                            foreach ($items_to_delete as $entity_id => $items_list)
                            {
                                foreach ($items_list as $id)
                                {
                                    items::delete($entity_id, $id);
                                }
                            }
                        ...
                        case strstr($actions['type'], 'copy_item_entity_'):
                            ...
                            $number_of_copies = $settigns->get('number_of_copies')>0 ? $settigns->get('number_of_copies') : 1;
                            $number_of_copies = isset($_POST['number_of_copies']) ? _POST('number_of_copies') : $number_of_copies;
                        ...
                        case strstr($actions['type'], 'repeat_item_entity_'):
                            ...
                            $sql_data_tasks = array(
                                'entities_id' => $action_entity_id,
                                'items_id' => $item_id,
                                'is_active' => 1,
                                'repeat_type' => $settigns->get('repeat_type'),
                                'repeat_time' => $settigns->get('repeat_time'),
                                'repeat_interval' => ($settigns->get('repeat_interval') > 0 ? $settigns->get('repeat_interval') : 1),
                                'repeat_days' => (($settigns->get('repeat_type') == 'weekly') ? implode(',', $settigns->get('repeat_days')) : ''),
                                'repeat_start' => (strlen($settigns->get('repeat_start')) ? get_date_timestamp($settigns->get('repeat_start')) : ''),
                                'repeat_end' => (strlen($settigns->get('repeat_end')) ? get_date_timestamp($settigns->get('repeat_end')) : ''),
                        ...
                        case strstr($actions['type'], 'clone_item_entity_'):
                            ...
                            {
                                clone_subitems::clone_nested_items_process($actions['id'], $item_id, (isset($_POST['parent_item_id']) ? _post::int('parent_item_id') : 0));
                            }
                            else
                            {                            
                        ...
                        case strstr($actions['type'], 'save_export_template_entity_'):
                            ...
                            if(is_array($save_export_template))
                            {
                                ...
                                require_once(CFG_PATH_TO_PHPSPREADSHEET);
            
                                require_once('includes/libs/PHPStep/0.2/vendor/autoload.php');
                                ...
                                foreach($save_export_template as $field_id => $templates)
                                {
                                    ...
                                    //prepare onlyoffice files
                                    if($app_fields_cache[$action_entity_id][$field_id]['type']=='fieldtype_onlyoffice')
                                    {
                                        $filenames = (new onlyoffice($action_entity_id))->copy_from_attachments($field_id, $filenames);
                                        //print_rr($filenames);
                                        //exit();                                        
                                    }                                    
                        ...
                        case strstr($actions['type'], 'insert_item_linked_entity_'):
                            ...
                            if($field_info = db_fetch_array($field_info_query))
                            {
                                ...
                                if($item_info = db_fetch_array($item_info_query))
                                {
                                    ...
                                    $sql_data['parent_item_id'] = ($app_entities_cache[$actions['entities_id']]['parent_id'] == $app_entities_cache[$action_entity_id]['parent_id'] ? $item_info['parent_item_id'] : 0);
                        ...
                        case strstr($actions['type'], 'edit_item_subentity_'):
                            ...
                            //get previous items info before update
                            $previous_items = [];
                            $previous_item_query = db_query("select * from app_entity_{$action_entity_id} where parent_item_id='" . db_input($item_id) . "'" . (strlen($filtered_items) ? ' and id in (' . $filtered_items . ')' : ''));
                            while($previous_item = db_fetch_array($previous_item_query))
                            {
                                $previous_items[$previous_item['id']] = $previous_item;
                            }
                            ...
                            $items_query = db_query("select * from app_entity_" . $action_entity_id . " where parent_item_id='" . db_input($item_id) . "'" . (strlen($filtered_items) ? ' and id in (' . $filtered_items . ')' : ''));
                            while($items = db_fetch_array($items_query))
                            {                                                                
                                ...
                                //send sms notification
                                $sms = new sms($action_entity_id, $items['id']);
                                $sms->send_to = false;
                                $sms->send_edit_msg($previous_items[$items['id']]);

                                //email rules
                                $email_rules = new email_rules($action_entity_id, $items['id']);
                                $email_rules->send_edit_msg($previous_items[$items['id']]);
                                
                                $log = new track_changes($action_entity_id, $items['id']);
                                $log->log_update($previous_items[$items['id']]);
                            }
                        ...
                        case strstr($actions['type'], 'insert_item_entity_'):

                            //prepare data before insert
                            $sql_data['parent_item_id'] = $_POST['parent_item_id']??0;
                            $sql_data['created_by'] = $app_user['id'];
                            $sql_data['date_added'] = time();
                            ...
                            if($action_entity_id==1 and isset($_POST['fields'][7]) and isset($_POST['fields'][8]) and isset($_POST['fields'][9]) and isset($_POST['fields'][12]))
                            {
                        ...
                        case strstr($actions['type'], 'edit_item_related_entity_'):
                            ...
                            //get previous items before update
                            $previous_items = [];
                            $previous_items_query = db_query("select * from app_entity_{$action_entity_id} where id in ({$where_sql})" . (strlen($filtered_items) ? ' and id in (' . $filtered_items . ')' : ''));
                            while($items = db_fetch_array($previous_items_query))
                            {
                                $previous_items[$items['id']] = $items;
                            }
                        ...
                        case strstr($actions['type'], 'insert_item_related_entity_'):
                            ...
                            //run actions after item insert
                            $processes = new processes($action_entity_id);
                            $processes->run_after_insert($related_items_id);
                        ...
                        case strstr($actions['type'], 'runphp_item_entity_'):
                            ...
                            $item_info_query = db_query("select e.* " . fieldtype_formula::prepare_query_select($action_entity_id). " from app_entity_" . $action_entity_id . " e where id='" . db_input($item_id) . "'");
                            $item_info = db_fetch_array($item_info_query);
                            ...
                            foreach($item_info as $field_id=>$field_value)
                            {
                                ...
                                if(is_null($field_value) or !strlen($field_value))
                                {
                                    $field_value = 0;
                                }
                            ...
                            if(strlen($php_code))
                            {
                                ...
                                {
                                    die(TEXT_ERROR . ' ' . TEXT_EXT_PROCESS . ': #' . $process_info['id'] . ', '  . TEXT_EXT_PROCESSES_ACTIONS . ': #' . $actions['id'] . ' - '. $e->getMessage() . ' on line ' . $e->getLine());
                                }
                            }
        ...
        if(!$is_ipn)
        {
            ...
            if($process_info['success_message_status']==1)
            {
                $alerts->add( strlen($process_info['success_message']) ? $process_info['success_message'] : sprintf(TEXT_EXT_PROCESS_COMPLETED, $process_info['name'], count($selected_items)), 'success');
            }
            
            //force print template
            if(strlen($process_info['print_template']) and count($selected_items)==1)
            {
                $app_force_print_template = $process_info['print_template'] . '_' . $process_info['entities_id'] . '_' . current($selected_items);
            }                        
            ...
            if(strstr($app_redirect_to, 'kanban'))
            {
                ...
                {
                    redirect_to('ext/kanban/view', 'id=' . str_replace('kanban', '', $app_redirect_to) . (isset($_GET['path']) ? '&path=' . $_GET['path']:''));
                }
            }
            
            if(strstr($app_redirect_to, 'ganttreport'))
            {                  
                redirect_to('ext/ganttchart/dhtmlx', 'id=' . str_replace('ganttreport', '', $app_redirect_to) . ($app_entities_cache[$process_info['entities_id']]['parent_id']>0 ? '&path=' . $app_path:''));
            }
    ...
    public function include_filtered_items($action_info, $parent_item_id = 0, $related_items_sql = '')
    {
        ...
        if($filters['total'] > 0 or ($settings->get('apply_entity_access_rules')==1 and (users::has_users_access_name_to_entity('view_assigned', $action_entity_id) or $force_access_check = users::has_users_access_name_to_entity('action_with_assigned', $action_entity_id))))
        {
            $listing_sql_query_select = '';
            $listing_sql_query = '';
            $listing_sql_query_having = '';
    ...
    {
        $value = str_replace(array('delete_item_entity_','insert_item_entity_','runphp_item_entity_','repeat_item_entity_','save_export_template_entity_', 'unlink_records_by_mysql_query_', 'link_records_by_mysql_query_', 'clone_item_entity_', 'clone_subitems_linked_entity_', 'move_item_entity_', 'edit_item_users_entity_1', 'insert_item_linked_entity_', 'edit_item_linked_entity_', 'edit_parent_item_entity_', 'edit_item_entity_', 'copy_item_entity_', 'edit_item_subentity_', 'insert_item_subentity_', 'edit_item_related_entity_', 'insert_item_related_entity_'), '', $type);
    ...
    public static function get_actions_types_choices($entities_id)
    {
        ...
        $choices['insert_item_entity_' . $entity_info['id']] = sprintf(TEXT_EXT_PROCESS_ACTION_INSERT_ITEM, $entity_info['name']);
        
        $choices['delete_item_entity_' . $entity_info['id']] = sprintf(TEXT_EXT_PROCESS_ACTION_DELETE_ITEM, $entity_info['name']);
        ...
        $choices['repeat_item_entity_' . $entity_info['id']] = sprintf(TEXT_EXT_PROCESS_ACTION_REPEAT_ITEM, $entity_info['name']);
        
        $choices['runphp_item_entity_' . $entity_info['id']] = sprintf(TEXT_EXT_PROCESS_ACTION_RUN_PHP, $entity_info['name']);
            ...
            switch($fields['type'])
            {
                case 'fieldtype_related_records':
                    ...
                    if($entity_id)
                    {
                        ...
                        $choices['unlink_records_by_mysql_query_' . $entity_id] = sprintf(TEXT_EXT_PROCESS_ACTION_UNLINK_RECORDS_BY_MYSQL_QUERY, $fields['name']);
                    }
                    break;
    ...
    public static function output_action_field_value($actions_fields)
    {
        ...
        if(in_array($actions_fields['field_type'], array('fieldtype_users_approve','fieldtype_users', 'fieldtype_users_ajax', 'fieldtype_created_by', 'fieldtype_dropdown', 'fieldtype_radioboxes', 'fieldtype_checkboxes','fieldtype_entity', 'fieldtype_entity_ajax', 'fieldtype_entity_multilevel')))
        {
        ...
        elseif(in_array($actions_fields['field_type'], array('fieldtype_input_date', 'fieldtype_input_date_extra', 'fieldtype_input_datetime')))
        {
        ...
        elseif(in_array($actions_fields['field_type'], array('fieldtype_input_file', 'fieldtype_attachments', 'fieldtype_image', 'fieldtype_image_ajax','fieldtype_3dviewer')))
        {
            return $actions_fields['value'];
        }
    ...
    function validate_sql_data($sql_data,$entity_id,$item_id)
    {
        ...
        if($entity_id==1)
        {
            if(isset($sql_data['field_12']))
            {
                ...
                $check_query = db_query("select id from app_entity_1 where field_12 = '" . db_input(db_prepare_input($sql_data['field_12'])) . "' and id!=" . $item_id);
            ...
            if(isset($sql_data['field_9']) and CFG_ALLOW_REGISTRATION_WITH_THE_SAME_EMAIL==0)
            {
                ...
                $check_query = db_query("select id from app_entity_1 where field_9 = '" . db_input(db_prepare_input($sql_data['field_9'])) . "' and id!=" . $item_id);
                if($check = db_fetch_array($check_query))
                {
