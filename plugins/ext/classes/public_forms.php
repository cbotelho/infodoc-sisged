...
class public_forms{
	...
	static function send_client_notification($entities_id,$prev_item_info,$is_comment=false)
	{
		...
		$forms_query = db_query("select f.*, e.name as entities_name from app_ext_public_forms f, app_entities e where e.id=f.entities_id and e.id='" . $entities_id . "' and notify_field_change>0");
	...
	static function prepare_email_content($public_form, $current_entity_id,$item_id, $email_type = 'new')
	{
		...
		$item_query = db_query("select e.* " . $listing_sql_query_select . " from app_entity_" . $current_entity_id . " e where id='" . $item_id . "'",false);
		...
		while($tabs = db_fetch_array($tabs_query))
		{
			...
			{
				$where_sql = (strlen($public_form['check_page_fields']) ? " and f.id in (" .  $public_form['check_page_fields'] . ")":' and f.id=0');
			}
			
			$fields_query = db_query("select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where f.type not in (" . fields_types::get_type_list_excluded_in_form(). ") {$where_sql} and f.entities_id='" . db_input($current_entity_id) . "' and f.forms_tabs_id=t.id and f.forms_tabs_id='" . db_input($tabs['id']) . "' order by t.sort_order, t.name, f.sort_order, f.name");
			while($field = db_fetch_array($fields_query))
			{
				...
				if(($cfg->get('hide_field_if_empty')==1 and strlen($value)==0) or (in_array($field['type'],array('fieldtype_created_by','fieldtype_input_date','fieldtype_input_datetime')) and $value==0))
				{
					continue;
				}
				...
				//prepare attachments
				if(in_array($field['type'],array('fieldtype_attachments','fieldtype_input_file')) and strlen($value))
				{
		...
		return array('html'=>$html,'attachments'=>$attachments,'item'=>$item);		
	}
	...
	{		
		...
		$listing_sql = "select * from app_comments where entities_id='" . db_input($entities_id) . "' and items_id='" . db_input($item_info['id']) . "' order by date_added ";
		...
		while($item = db_fetch_array($items_query))
		{
			...
			while($field = db_fetch_array($comments_fields_query))
			{
				...
				if(!in_array($field['id'],explode(',',$public_form['check_page_comments_fields']))) continue;
        ...
        <tr><th>&bull;&nbsp;' . $field['name'] . ':&nbsp;</th><td>' . fields_types::output($output_options). '</td></tr>
			...
			$output_options = array(
					...
					'path'=>$entities_id,
					'is_public_form'=>$public_form['id'],
					'field'=>array('entities_id'=>$entities_id,'id'=>$item['id'] . '-comment','configuration'=>''),
					'item'=>array('id'=>$item_info['id']));
			
			$attachments = fields_types::output($output_options);
			
			if(strlen($attachments)) $attachments = '<br>' . $attachments ;
		...
		{
			$html = '
					<h4>' . (strlen($public_form['check_page_comments_heading']) ? $public_form['check_page_comments_heading']:TEXT_COMMENTS) . '</h4>
					<table class="table table-striped table-bordered table-hover" >
						' . $html . '
					</table>';
		}
		
		return array('html'=>$html, 'attachments'=>$attachments_array);		
	}
	...
	static function send_notifictaion($public_form,$item, $choices_id)
	{
		if(strlen($public_form['customer_name']) and strlen($public_form['customer_email']))
		{
			...
			$email_content = self::prepare_email_content($public_form, $public_form['entities_id'],$item['id'],'notify');
			...
			if(app_validate_email($customer_email))
			{
				...
				$subject = (strlen($notify_message_title) ? $notify_message_title : TEXT_DEFAULT_EMAIL_SUBJECT_UPDATED_ITEM . ' ' . $item_name);
				...
				$body = users::use_email_pattern('single_column',array('email_single_column'=>$body));
