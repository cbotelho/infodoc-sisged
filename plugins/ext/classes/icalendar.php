...
class icalendar
{
    ...
    function export_public()
    {
        ...
        $events_query = db_query("select * from app_ext_calendar_events where (event_type='public' or (event_type='personal' and is_public=1)) order by start_date");
    ...
    function export_personal()
    {
        ...
        $events_query = db_query("select * from app_ext_calendar_events where event_type='personal' and users_id='" . db_input($app_user['id']) . "' order by start_date");
        while ($events = db_fetch_array($events_query))
        {
            $event_start = date('Y-m-d H:i:s',$events['start_date']);
            $event_end = date('Y-m-d H:i:s',$events['end_date']); 
    ...
    function export_pivot_report()
    {
        ...
        $reports_query = db_query("select * from app_ext_pivot_calendars where id='" . db_input($this->reports_id) . "' and enable_ical=1");
        ...
        while($reports_entities = db_fetch_array($reports_entities_query))
        {
            ...
            while($events = db_fetch_array($events_query))
            {
                ...
                if(strstr($event_end,' 00:00:00'))
                {
                  $event_end = date('Y-m-d H:i:s',strtotime('+1 day',$events['field_' . $reports_entities['end_date']]));
                }
    ...
    function export_report()
    {
        ...
        $reports_query = db_query("select * from app_ext_calendar where id='" . db_input($this->reports_id) . "' and enable_ical=1");
        if(!$reports = db_fetch_array($reports_query))
        {
            die(TEXT_NO_RECORDS_FOUND);
        }
        ...
        $fiters_reports_id = default_filters::get_reports_id($reports['entities_id'], 'calendarreport' . $reports['id']);
    ...
    static function get_url($is_enabled, $type, $reports_id=0)
    {
        if($is_enabled)
        {
            return '&nbsp;<a class="ical-export-url" href="javascript: open_dialog(\'' . url_for('ext/calendar/ical','type=' . $type . ($reports_id>0 ? '&id=' . $reports_id : '') . (isset($_GET['path']) ? '&path=' . $_GET['path'] : '')). '\')" title="' . TEXT_EXT_SHARE_THIS_CALENDAR . '"><i class="fa fa-rss" aria-hidden="true"></i></a>';
        }
    }
    ...
    static function generate_url($type, $reports_id=0)
    {
        ...
        $url = url_for('feeders/ical','GMT=0&type=' . $type  . '&client=' . $app_user['client_id'] . ($reports_id>0 ? '&id=' . $reports_id : '') . (isset($_GET['path']) ? '&path=' . $_GET['path'] : ''));
