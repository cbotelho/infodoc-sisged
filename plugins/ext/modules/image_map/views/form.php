...
<?php echo form_tag('configuration_form', url_for('ext/image_map/reports','action=save' . (isset($_GET['id']) ? '&id=' . $_GET['id']:'') ),array('class'=>'form-horizontal')) ?>
...
?>    
    <div class="form-group">
    	<label class="col-md-4 control-label" for="type"><?php echo TEXT_SCALE ?></label>
      <div class="col-md-8">	
    	  <?php echo select_tag('scale',$choices, $obj['scale'],array('class'=>'form-control input-small')) ?>        
      </div>			
    </div>
...
{ 
  ...
  $('#reports_entities_fields').load('<?php echo url_for("ext/image_map/reports","action=get_entities_fields")?>',{entities_id:entities_id, id:'<?php echo $obj["id"] ?>'},function(response, status, xhr) {
    if (status == "error") {                                 
