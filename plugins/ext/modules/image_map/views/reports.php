...
<h3 class="page-title"><?php echo TEXT_EXT_IMAGE_MAP ?></h3>

<p><?php echo TEXT_EXT_IMAGE_MAP_DESCRIPTION ?></p>

<?php echo button_tag(TEXT_BUTTON_CREATE,url_for('ext/image_map/form')) ?>
...
?>
<tr>
  <td style="white-space: nowrap;"><?php echo button_icon_delete(url_for('ext/image_map/delete','id=' . $reports['id'])) . ' ' . button_icon_edit(url_for('ext/image_map/form','id=' . $reports['id'])) ?></td>
  ...
  <td><?php echo link_to($reports['name'], url_for('ext/image_map/view','id=' . $reports['id'])) ?></td>
