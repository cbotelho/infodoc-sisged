...
<?php
  ...
  while($items = db_fetch_array($items_query))     
  {
    ...
    echo '<div>' . input_checkbox_tag('items[]',$items['id'],array('class'=>'remove_related_item')) . ' <a href="' . url_for('items/info', 'path=' . $path_info['full_path']) . '" target="_blank">' . items::get_heading_field($related_entities_id,$items['id']) . '</a></div>';
  }
