...
switch($app_module_action)
{
	...
	case 'get_body':		
		...
		$mail_info_query = db_query("select body,id,attachments,groups_id from app_ext_mail where id='" . _get::int('mail_id'). "'");
		if($mail_info = db_fetch_array($mail_info_query))
		{
			$custom_head = '
					...
					$(function(){
						...
						$(".button-more").click(function(){
							...
							window.parent.postMessage({"height":height, "id":' . $mail_info['id'] . '}, "*")
                                                ...
                                                //revmoe absolute style because there is issue with detet body height
			...
			';
                        ...
                        if(is_file($custom_css = 'css/skins/' . $app_skin_dir . '/mail_body.css'))
                        {                            
                            $custom_head .= '<link href="' . $custom_css . '" rel="stylesheet" type="text/css" />';
                        }
