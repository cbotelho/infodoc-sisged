...
while($item = db_fetch_array($items_query))
{
    ...
    {
        $body_html = preg_replace('#<style(.*?)>(.*?)</style>#is', '', $last_item['body']);
        $body_short = strip_tags((strlen($last_item['body_text']) ? $last_item['body_text'] : $body_html));
        $body_short = mb_substr($body_short, 0, 160);
    }
    ...
    else
    {
        ...
        $html .= '
			<tr ' . $css . ' >
				...
				' . (($count_accounts > 2 or $app_mail_filters['folder'] == 'sent') ? '<td ' . $onClick . '>' . render_bg_color_block($item['bg_color'], $to_email) . '</td>' : '') . '
...
<table width="100%" class="listing-split-page">
    <tr>
