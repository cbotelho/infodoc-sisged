...
switch($app_module_action)
{
    case 'add_row':
        ...
        {
            $sort_order_query = db_query("select (min(sort_order)-1) as sort_order from app_ext_report_page_blocks where block_type='" . $_GET['block_type'] . "' and report_id=" . $report_page['id'] . " and parent_id=" . $block_info['id']);
        }
        else
        {
            $sort_order_query = db_query("select (max(sort_order)+1) as sort_order from app_ext_report_page_blocks where block_type='" . $_GET['block_type'] . "' and report_id=" . $report_page['id'] . " and parent_id=" . $block_info['id']);            
        }
        ...
        redirect_to('ext/report_page/' . $block_cfg_page ,'block_id=' . $block_info['id']);
        break;
    case 'delete_row':
        if(isset($_GET['id']))
        {                        
    ...
    case 'save':
        ...
        redirect_to('ext/report_page/extra_rows','report_id=' . $report_page['id'] . '&block_id=' . $block_info['id'] . '&row_id=' . $row_info['id']);
        break;
    case 'delete':
        if(isset($_GET['id']))
        {
