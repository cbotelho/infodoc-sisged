...
<?php
  ...
  else
  {
    ...
    elseif($reports['filters_panel']=='quick_filters')
    {
        ...
        $filters_panels->set_type('gantt_reports' . $reports['id']);
        $filters_panels->set_items_listing_funciton_name('load_gantt_report');
        echo '<div class="gantt_reports' . $reports['id'] . '">' . $filters_panels->render_horizontal() . '</div>';
    }
...
//refresh gantt by filter panel
function load_gantt_report()
{    
    gantt.clearAll();
    gantt.load(gantt_load_url);
    gantt.refreshData();
