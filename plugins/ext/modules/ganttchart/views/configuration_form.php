        ...
        <div class="tab-content">
            ...
            <div class="tab-pane fade" id="settings">    
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-9">	
                        <?php echo select_tag('gantt_date_format', $format_list, $obj['gantt_date_format'], array('class' => 'form-control input-medium required')) ?>
                        <?php echo tooltip_text(TEXT_EXT_GANTT_DATE_FORMAT_INFO) ?>        
                    </div>			
                </div>

                <?php
                $choices = ['hour' => TEXT_EXT_HOUR, 'day' => TEXT_EXT_DAY, 'week' => TEXT_EXT_WEEK, 'month' => TEXT_EXT_MONTH, 'year' => TEXT_EXT_YEAR];
                ?>    
                <div class="form-group">
                ...
                <?php
                $choices = ['terrace' => 'Terrace', 'skyblue' => 'Skyblue', 'meadow' => 'Meadow', 'broadway' => 'Broadway', 'material' => 'Material', 'contrast_white' => 'High contrast light','contrast_black' => 'Contrast Black'];
                ?>     
                <div class="form-group">
                ...
                <div id="settings_fields_width"></div>
...
<script>
    ...
    function entity_listing_fields()
    {
        $('#entity_listing_fields').load('<?php echo url_for("ext/ganttchart/configuration", "action=get_entity_listing_fields") ?>', {entities_id: $('#entities_id').val(), id: '<?php echo $obj["id"] ?>'}, function (response, status, xhr)
        {
            ...
            else
            {
                ...
                $('#fields_in_listing').change(function ()
                {
                    load_settings_fields_width()
                })
    ...
    function load_settings_fields_width()
    {
        $('#settings_fields_width').load(url_for("ext/ganttchart/configuration", "action=settings_fields_width"), {entities_id: $('#entities_id').val(),id: '<?php echo $obj["id"] ?>', fields: $("#fields_in_listing").val(), settings: $('.settings_width').serialize()})
    }
