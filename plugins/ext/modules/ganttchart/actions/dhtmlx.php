...
$reports_query = db_query("select * from app_ext_ganttchart where is_active=1 and id='" . db_input((int) $_GET['id']) . "'");
...
switch($app_module_action)
{
            ...
            case 'tasks':
                ...
                switch($_POST['!nativeeditor_status'])
                {
                    ...
                    case 'updated':
                        ...
                        {
                            $sql_data['field_' . $reports['end_date']] = strtotime("-1 hour", get_date_timestamp($_POST['end_date']));
                        }
    ...
    case 'load_data':
        ...
        {
            $panel_fiters_reports_id = reports::auto_create_report_by_type($reports['entities_id'], 'panel_gantt_reports' . $reports['id'], true);
            $listing_sql_query = reports::add_filters_query($panel_fiters_reports_id, $listing_sql_query);
        }
        
        //default filters
        $default_fiters_reports_id = default_filters::get_reports_id($reports['entities_id'], 'default_gantt_reports' . $reports['id']);
        ...
        if($app_fields_cache[$reports['entities_id']][$reports['start_date']]['type'] != 'fieldtype_dynamic_date' and !ganttchart::has_date_added($reports))
        {
        ...
        {
            $listing_sql_query_select .= ", (select min(parent." . (ganttchart::has_date_added($reports) ? 'date_added':'field_' . $reports['start_date']). ") from app_entity_{$reports['entities_id']} as parent where parent.parent_item_id=e.parent_item_id) as parent_min_start_date";
            $listing_order_query = " parent_min_start_date, ";

            $display_parent_item = true;
        }

        $listing_sql = "select e.* " . $listing_sql_query_select . " from app_entity_" . $reports['entities_id'] . " e  where  e.id>0 " . $listing_sql_query . " order by " . $listing_order_query . " e.sort_order," . (ganttchart::has_date_added($reports) ? "e.date_added":"e.field_" . $reports['start_date']);
