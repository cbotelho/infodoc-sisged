...
switch($app_module_action)
{
        ...
        if(strlen($_POST['keywords']))
        {
            if(app_parse_search_string($_POST['keywords'], $search_keywords))
            {
                ...
                if(isset($search_keywords) && (sizeof($search_keywords) > 0))
                {
                    ...
                    $messages_query = db_query("select * from app_ext_chat_conversations_messages where conversations_id='" . $assigned_to . "' and " . $where_str . " order by id");
                    while($messages = db_fetch_array($messages_query))
                    {
                        echo $app_chat->render_message_template($messages, $assigned_to, true);
                    }
                }
            }
        }
    ...
    case 'get_messages':
        ...
        $messages_ids = array();
        $count = 0;
        $messages_query = db_query("select * from app_ext_chat_conversations_messages where conversations_id='" . $assigned_to . "' and id>" . (int) $app_users_cfg->get('app_chat_last_msg_id') . " order by id desc");
        while($messages = db_fetch_array($messages_query))
        {
            $html = $app_chat->render_message_template($messages, $assigned_to, true) . $html;
    ...
    case 'get_previous_messages':
        ...
        $messages_query = db_query("select * from app_ext_chat_conversations_messages where conversations_id='" . $assigned_to . "' order by id desc " . $sql_query);
        while($messages = db_fetch_array($messages_query))
        {
