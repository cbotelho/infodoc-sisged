...
<?php echo form_tag('templates_form', url_for('ext/templates_xlsx/entity_blocks','templates_id=' . _GET('templates_id') . '&action=save' . (isset($_GET['id']) ? '&id=' . $_GET['id']:'')  . '&parent_block_id=' . $parent_block['id']),array('class'=>'form-horizontal')) ?>
...
<script>
  ...
  { 
    ...
    $('#block_settings').load('<?php echo url_for("ext/templates_xlsx/entity_blocks_settings",'templates_id=' . _GET('templates_id') ) ?>',{fields_id:fields_id, id:'<?php echo $obj["id"] ?>'},function(response, status, xhr) {
      if (status == "error") {                                 
