...
<?php echo form_tag('reports_filters', url_for('ext/pivot_map_reports/filters','action=save&map_reports_id=' . _get::int('map_reports_id'). '&reports_id=' . $_GET['reports_id'] . (isset($_GET['parent_reports_id']) ? '&parent_reports_id=' . $_GET['parent_reports_id']:'').  (isset($_GET['id']) ? '&id=' . $_GET['id']:'') ),array('class'=>'form-horizontal')) ?>
