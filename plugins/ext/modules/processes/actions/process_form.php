...
switch($app_module_action)
{
    ...
    case 'sort_fields':
        //print_r($_POST);
        $tabs_query = db_fetch_all('app_ext_process_form_tabs', "process_id='" . db_input($_GET['process_id']) . "' order by  sort_order, name");
        while($tabs = db_fetch_array($tabs_query))
        {
            if(isset($_POST['forms_tabs_' . $tabs['id']]))
            {
                db_query("update app_ext_process_form_tabs set fields='" . str_replace('form_fields_','',$_POST['forms_tabs_' . $tabs['id']]) . "' where id=" . $tabs['id']);                                
            }

            //handle rows
            $rows_query = db_query("select * from app_ext_process_form_rows where process_id='" . _GET('process_id') . "' and forms_tabs_id='" . $tabs['id'] . "' order by sort_order");
            while($rows = db_fetch_array($rows_query))
            {
                for($i = 1; $i <= $rows['columns']; $i++)
                {
                    ...
                    {
                        db_query("update app_ext_process_form_rows set column{$i}_fields='" . str_replace('form_fields_','',$_POST['forms_rows_' . $tabs['id'] . '_' . $rows['id'] . '_' . $i]) . "' where id=" . $rows['id']);                                                
                    }
                }
            }
        }
    ...
    case 'sort_tabs':
        if(isset($_POST['forms_tabs_ol']))
        {
            ...
            {
                db_perform('app_ext_process_form_tabs', array('sort_order' => $sort_order), 'update', "id='" . db_input($v) . "'");
                $sort_order++;
            }
        }        
        exit();
    ...
    case 'save_row':               
        ...
        {   
            $check_query = db_query("select (max(sort_order)+1) as total from app_ext_process_form_rows where process_id='" . _GET('process_id'). "' and forms_tabs_id='" . _GET('forms_tabs_id'). "'");
    ...
    case 'sort_rows':
        ...
        while($tabs = db_fetch_array($tabs_query))
        {
            if(isset($_POST['forms_rows_' . $tabs['id']]))
            {
                ...
                {
                    db_perform('app_ext_process_form_rows',array('sort_order'=>$sort_order,'forms_tabs_id'=>$tabs['id']),'update',"id='" . db_input($v) . "'");
                    $sort_order++;                                        
                }
            }
        }
