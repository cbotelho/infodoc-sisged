...
<?php echo form_tag('reports_filters', url_for('ext/item_pivot_tables/filters','action=save&pivot_reports_id=' . _get::int('pivot_reports_id') . '&reports_id=' . $_GET['reports_id'] . (isset($_GET['parent_reports_id']) ? '&parent_reports_id=' . $_GET['parent_reports_id']:'').  (isset($_GET['id']) ? '&id=' . $_GET['id']:'') ),array('class'=>'form-horizontal')) ?>
...
function load_fitlers_options(fields_id)
{
  ...
  $('#filters_options').load('<?php echo url_for("reports/filters_options")?>',{fields_id:fields_id,is_internal:1, id:'<?php echo $obj["id"] ?>'},function(response, status, xhr) {
    if (status == "error") {                                 
