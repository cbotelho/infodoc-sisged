...
$pivot_reports_query = db_query("select * from app_ext_item_pivot_tables where id='" . _get::int('pivot_reports_id') . "'");
if(!$pivot_reports = db_fetch_array($pivot_reports_query))
{
	redirect_to('ext/item_pivot_tables/reports');
}
...
switch($app_module_action)
{
  case 'save':
    ...
    redirect_to('ext/item_pivot_tables/filters','reports_id=' . $_GET['reports_id'] . '&pivot_reports_id=' . $pivot_reports['id']);
