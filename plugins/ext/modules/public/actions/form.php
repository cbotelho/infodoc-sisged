...
if(defined('AVAILABLE_PLUGINS'))
{
    ...
    {
        if(is_file($v = 'plugins/' . $plugin . '/public_form_action.php'))
        {
            require($v);
        }
    }
...
switch($app_module_action)
{
    ...
    case 'save':
        ...
        if(!$is_error)
        {
            ...
            if(strlen($public_form['admin_name']) and strlen($public_form['admin_email']) and $public_form['admin_notification'] == 1)
            {
                $html = users::use_email_pattern('single_column', array('email_single_column' => $html));
             ...
             * */
            //success msg
            $public_form_success_msg = (strlen($successful_sending_message) ? $successful_sending_message : TEXT_EXT_PB_SUCCESSFUL_SENDING_MESSAGE_DEFAULT);
    ...
    case 'check_unique':
        ...
        echo items::check_unique(_get::int('entities_id'), _post::int('fields_id'), $_POST['fields_value'], false, $unique_for_each_parent);

        exit();
        break;
    case 'check_unique_rule':
        
        echo composite_unique_fields::check(_get::int('entities_id'));
        exit();
        break;
