...
if(defined('AVAILABLE_PLUGINS'))
{
    ...
    {
        if(is_file($v = 'plugins/' . $plugin . '/public_form_view.php'))
        {
            require($v);
        }
    }
    ...
    if($entity_info['parent_id'] > 0)
    {
        ...
        else
        {
            ...
            <div class="form-group form-group-parent-item-id" >
                ...
                <div class="col-md-9">	
        <?php 
            if(($parent_id = $app_entities_cache[$public_form['entities_id']]['parent_id'])>0)
            {
                $reports_id = reports::get_reports_id_by_type($parent_id, 'public_form_add_in' . $public_form['id']);  
            }
            else
            {
                $reports_id = false;
            ...
            $items_query = new items_query($entity_info['parent_id'],[
                'add_formula'=>true,                
            ...
            $items_query = db_query($items_query->get_sql());
            while ($item = db_fetch_array($items_query))
            {
                $path_info = items::get_path_info($entity_info['parent_id'], $item['id']);                
                $parent_name = '';
                ...
                $choices[$item['id']] = $parent_name . items::get_heading_field($entity_info['parent_id'], $item['id']);
            }
            
            echo select_tag('parent_item_id', $choices, $parent_item_id, array('class' => 'form-control chosen-select required'));
...
if($count_tabs > 1)
{
    ...
    foreach($tabs_tree as $tabs)
    {
      ...
      ' . (strlen($tabs['description']) ? '<p>' . $tabs['description'] . '</p>' : '');
        ...
        //add recaptcha to last tabl
        if($count_tabs==count($tabs_tree)-1)
        {
            $html_tab_content[$tabs['id']] .= $recaptcha_html . $user_agreement_html;
        }
...
echo $html;
    ...
    {
        echo $recaptcha_html . $user_agreement_html;
    }   

    $button_save_title = (strlen($public_form['button_save_title']) > 0 ? $public_form['button_save_title'] : TEXT_BUTTON_SEND);
...
<?php echo forms_fields_rules::hidden_form_fields($current_entity_id, false) ?>
