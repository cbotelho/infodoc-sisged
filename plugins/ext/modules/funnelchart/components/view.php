...
while($item = db_fetch_array($items_query))
{
	...
	{	
		...
		{	
			//exclude choices
			if(in_array($value,explode(',',$reports['exclude_choices'])) or !isset($funnel_info_choices[$value])) continue;
...
foreach($funnel_info_choices as $choices_id=>$value)
{
	...
	$data = "
  		...
  		y: " . $value['count'] . ",
                colorIndex: {$count},    
  		" . ((isset($choices_backgrounds[$choices_id]) and !strlen($color)) ? "color: '" .  $choices_backgrounds[$choices_id]['background'] . "',":'') . "
