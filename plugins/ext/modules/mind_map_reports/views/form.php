...
<?php echo form_tag('configuration_form', url_for('ext/mind_map_reports/reports','action=save' . (isset($_GET['id']) ? '&id=' . $_GET['id']:'') ),array('class'=>'form-horizontal')) ?>
...
?>    
    <div class="form-group">
    	<label class="col-md-4 control-label" for="type"><?php echo TEXT_SHAPE ?></label>
      <div class="col-md-8">	
    	  <?php echo select_tag('shape',$choices, $obj['shape'],array('class'=>'form-control input-medium')) ?>        
      </div>			
    </div>
...
{ 
  ...
  $('#reports_entities_fields').load('<?php echo url_for("ext/mind_map_reports/reports","action=get_entities_fields")?>',{entities_id:entities_id, id:'<?php echo $obj["id"] ?>'},function(response, status, xhr) {
    if (status == "error") {                                 
...
{  
  ...
  $('#reports_entities_fields_icons').load('<?php echo url_for("ext/mind_map_reports/reports","action=get_entities_fields_icons")?>',{entities_id:entities_id, id:'<?php echo $obj["id"] ?>',fields_id:fields_id},function(response, status, xhr) {
    if (status == "error") {                                 
       $(this).html('<div class="alert alert-error"><b>Error:</b> ' + xhr.status + ' ' + xhr.statusText+'<div>'+response +'</div></div>')                    
    }
    else
