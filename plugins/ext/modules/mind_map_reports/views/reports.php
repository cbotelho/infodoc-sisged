...
<h3 class="page-title"><?php echo TEXT_EXT_MIND_MAP ?></h3>

<p><?php echo TEXT_EXT_MIND_MAP_DESCRIPTION ?></p>

<?php echo button_tag(TEXT_BUTTON_CREATE,url_for('ext/mind_map_reports/form')) ?>
...
?>
<tr>
  <td style="white-space: nowrap;"><?php echo button_icon_delete(url_for('ext/mind_map_reports/delete','id=' . $reports['id'])) . ' ' . button_icon_edit(url_for('ext/mind_map_reports/form','id=' . $reports['id'])) ?></td>
  <td><?php echo $app_entities_cache[$reports['entities_id']]['name'] ?></td>  
  <td><?php echo ( $app_entities_cache[$reports['entities_id']]['parent_id']==0 ? link_to($reports['name'], url_for('ext/mind_map_reports/view','id=' . $reports['id'])) : $reports['name']) ?></td>
