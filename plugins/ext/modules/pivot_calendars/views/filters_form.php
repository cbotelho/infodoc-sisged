...
<?php echo form_tag('reports_filters', url_for('ext/pivot_calendars/filters','action=save&calendars_id=' . _get::int('calendars_id'). '&reports_id=' . $_GET['reports_id'] . (isset($_GET['parent_reports_id']) ? '&parent_reports_id=' . $_GET['parent_reports_id']:'').  (isset($_GET['id']) ? '&id=' . $_GET['id']:'') ),array('class'=>'form-horizontal')) ?>
