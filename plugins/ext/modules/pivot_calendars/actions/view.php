...
{	
	case 'resize':
		...
		$itesm_id = (int)$id_info[1];
                ...
                $item_info = db_find("app_entity_{$entities_id}",$itesm_id);
		...
		{	
			...
			db_perform("app_entity_" . $entities_id ,$sql_data,'update',"id='" . $itesm_id . "'");
                        ...
                        $log = new track_changes($entities_id, $itesm_id);
	...
	case 'get_events':
		...
		{				
			...
			while($events = db_fetch_array($events_query))
			{
				...
				$color = (strlen($reports_entities['bg_color']??'') ? $reports_entities['bg_color'] : '#3a87ad');
				if($reports_entities['use_background']>0)
				{
