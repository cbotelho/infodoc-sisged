...
switch($app_module_action)
{
	case 'save':
		...
		if(isset($_GET['id']))
		{
			db_perform('app_ext_recurring_tasks',$sql_data,'update',"id='" . db_input($_GET['id']) . "' and created_by='" . $app_user['id']. "'");
		}

		redirect_to('ext/recurring_tasks/my_recurring_tasks');

		break;
	case 'delete':

		$tasks_query = db_query("select id from app_ext_recurring_tasks where id='" . db_input($_GET['id']) . "' and created_by='" . $app_user['id']. "'");
