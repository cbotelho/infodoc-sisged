...
switch($app_module_action)
{
    case 'add_row':
        ...
        {
            $sort_order_query = db_query("select (min(sort_order)-1) as sort_order from app_ext_export_selected_blocks where block_type='" . $_GET['block_type'] . "' and templates_id=" . $template_info['id'] . " and parent_id=0");
        }
        else
        {
            $sort_order_query = db_query("select (max(sort_order)+1) as sort_order from app_ext_export_selected_blocks where block_type='" . $_GET['block_type'] . "' and templates_id=" . $template_info['id'] . " and parent_id=0");
        }
        ...
        if(isset($_GET['id']))
        {
            db_perform('app_ext_export_selected_blocks', $sql_data, 'update', "id='" . db_input($_GET['id']) . "'");
        }
        else
        {
            db_perform('app_ext_export_selected_blocks', $sql_data);
        }

        redirect_to('ext/export_selected/table_blocks', 'templates_id=' . $template_info['id'] );
        break;
    case 'delete_row':
        if(isset($_GET['id']))
        {
            export_selected::delele_block(_GET('id'));
    ...
    case 'save':
        ...
        redirect_to('ext/export_selected/extra_rows', 'templates_id=' . $template_info['id'] . '&row_id=' . $row_info['id']);
        break;
    case 'delete':
        if(isset($_GET['id']))
        {
