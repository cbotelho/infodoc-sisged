...
<?php echo form_tag('templates_form', url_for('ext/export_selected/table_blocks','templates_id=' . _GET('templates_id') . '&action=save' . (isset($_GET['id']) ? '&id=' . $_GET['id']:'')),array('class'=>'form-horizontal')) ?>
...
$field_entity_id = $template_info['entities_id'];
...
$choices = [];
$fields_query = fields::get_query($field_entity_id," and f.type not in ('fieldtype_action','fieldtype_parent_item_id') and f.id not in (select fields_id from app_ext_items_export_templates_blocks where block_type = 'body_cell' and templates_id=" . $template_info['id'] . " " . ($obj['fields_id']>0 ? " and fields_id!=" . $obj['fields_id'] :"") . "  and parent_id=0)");
...
<script>
  ...
  {
	 $('#field_settings').load("<?php echo url_for('ext/export_selected/table_blocks','id=' . $obj['id'] . '&action=get_field_settings&templates_id=' . $template_info['id'] . '&parent_block_id=0') ?>",{fields_id:$('#fields_id').val()},function(){ appHandleUniform(); })  
  }
</script>