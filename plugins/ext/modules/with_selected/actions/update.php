    ...
    case 'get_field_values':
        ...
        switch ($field_info['type'])
        {
            case 'fieldtype_input':
                $html = input_tag('values[' . $fields_id . ']', '', array('class' => 'form-control'));
                break;
            case 'fieldtype_textarea':
                $html = textarea_tag('values[' . $fields_id . ']', '', array('class' => 'form-control'));
                break;
            ...
            case 'fieldtype_entity_ajax':
                ...
                elseif ($field_cfg->get('display_as') == 'dropdown_muliple' or $field_cfg->get('display_as') == 'dropdown_multiple' or $field_cfg->get('display_as') == 'checkboxes')
                {
                    $attributes = array('class' => 'form-control chosen-select input-large',
                        'multiple' => 'multiple',
            ...
            case 'fieldtype_user_status':
                $html = select_tag('values[' . $fields_id . ']',array('1'=>TEXT_ACTIVE,'0'=>TEXT_INACTIVE),'',array('class'=>'form-control input-medium')) . tooltip_text(TEXT_FIELDTYPE_USER_STATUS_TOOLTIP);
                break;
            ...
            case 'fieldtype_users':
                ...
                if ($field_cfg->get('display_as') == 'checkboxes' or $field_cfg->get('display_as') == 'dropdown_muliple' or $field_cfg->get('display_as') == 'dropdown_multiple')
                {
                    $attributes['multiple'] = 'multiple';
        ...
        if (count($app_selected_items[$_GET['reports_id']]) > 0)
        {
                ...
                foreach ($_POST['fields_id'] as $fields_id)
                {
                    ...
                    {                        
                        ...
                        case 'fieldtype_dropdown':
                            ...
                            if ($value != $item_info['field_' . $fields_id] and $field_cfg->get('notify_when_changed') == 1 and isset($app_choices_cache[$value]))
                            {
                                $app_changed_fields[] = array(
            ...
            switch ($app_redirect_to)
            {
                ...
                default:
                    ...
                    {
                        $redirect_to = url_for('ext/mail/info', 'id=' . str_replace('mail_info_page_', '', $app_redirect_to));
                    }
