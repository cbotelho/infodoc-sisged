...
switch($app_module_action)
{
	case 'save':
		...
		if(isset($_GET['id']))
		{
			db_perform('app_ext_mail_accounts_entities_filters',$sql_data,'update',"id='" . db_input($_GET['id']) . "'");
		}
		else
		{
			db_perform('app_ext_mail_accounts_entities_filters',$sql_data);
		}
		
		redirect_to('ext/mail_integration/entities_filters','account_entities_id=' . $accounts_entities['id']);
		break;
	case 'delete':
		
		db_delete_row('app_ext_mail_accounts_entities_filters',_get::int('id'));
