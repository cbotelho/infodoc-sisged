...
switch($app_module_action)
{
          ...
          <label class="col-md-3 control-label" for="type">' . TEXT_EXT_LIST_OF_CONTACTS . '</label>
                <div class="col-md-9">
                  ...
                  <div class="input-group input-xlarge">
                    ' . select_tag('contact_list_id_choices', ['' => ''] + $choices, $obj['contact_list_id'], array('class' => 'form-control input-medium required')) . '
                    <span class="input-group-btn">
                    ' . input_tag('contact_list_id', $obj['contact_list_id'], ['class' => 'form-control input-medium', 'placeholder' => TEXT_ID]) . '
                    </span>
                  </div>      
    ...
    case 'get_entities_fields':
        ...
        $choices = array();
        $fields_query = db_query("select f.*, t.name as tab_name from app_fields f, app_forms_tabs t where f.type in ('fieldtype_user_email','fieldtype_input_email','fieldtype_mysql_query','fieldtype_php_code') and f.entities_id='" . $entities_id . "' and f.forms_tabs_id=t.id order by t.sort_order, t.name, f.sort_order, f.name");
