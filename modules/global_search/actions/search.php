...
switch($app_module_action)
{
    case 'listing':
        ...
        {
            if(CFG_GLOBAL_SEARCH_IN_INFO_PAGES==1 and app_parse_search_string($_POST['keywords'], $search_keywords))
            {
                $sql = "select id, name as title,'ipages' as entities_id  from app_ext_ipages where is_menu=0 and (find_in_set(" . $app_user['group_id'] . ",users_groups) or find_in_set(" . $app_user['id'] . ",assigned_to))";
                ...
                for($i = 0, $n = sizeof($search_keywords); $i < $n; $i++)
                {
                    switch($search_keywords[$i])
                    {
                        ...
                        default:
                            ...
                            $sql .= "name like '%" . db_input($keyword) . "%' or description like '%" . db_input($keyword) . "%'";
                            break;
                    }
                }
                $sql .= ")";
            ...
            $where_sql = (isset($_POST['search_by_entities']) ? " and gs.entities_id in (" . implode(',', $_POST['search_by_entities']) . ")" : "");
            ...
            while($entities = db_fetch_array($entities_query))
            {
                ...
                if(!in_array($app_fields_cache[$entities['entities_id']][$heading_field_id]['type'],['fieldtype_date_added','fieldtype_created_by']))
                {
                ...
                if(count($where_sql))
                {
                    $sql = "select {$fields_in_listing} from app_entity_{$entities['entities_id']} e where (" . (implode(' or ', $where_sql)) . ") ";
