...
switch($app_module_action)
{
    ...
    case 'sort':
        ...
        $groups_query = db_query("select * from app_forms_tabs where is_folder=1 and entities_id='" . _GET('entities_id') . "' order by sort_order, name");                
        while($groups = db_fetch_array($groups_query))
        {
            if(isset($_POST['group_' . $groups['id']]))
            {
              ...
              {
                db_perform('app_forms_tabs',array('sort_order'=>$sort_order,'parent_id'=>$groups['id']),'update',"id='" . db_input($v) . "'");
                $sort_order++;
              }
            }
        }
