...
<div class="table-scrollable ">
  <div class="table-scrollable table-wrapper slimScroll" id="slimScroll">
    <table class="tree-table table table-striped table-bordered table-hover ' . ($listing->is_resizable() ? 'table-resizable':''). '"  data-fixed-head="1" data-resizable="' . $listing->is_resizable(). '" ' . $listing->resizable_table_widht(). '>
      <thead>
        <tr>
          ' . ($has_with_selected ? '<th class="multiple-select-action-th">' . input_checkbox_tag('select_all_items', $_POST['reports_id'], array('class' => 'select_all_items','is_tree_view'=>1)) . '</th>' : '');
...
while($item = db_fetch_array($items_query))
{
    ...
    foreach($listing_fields as $field)
    {
        ...
        $editable_listing = new editable_listing($current_entity_id, $item, $field,$fields_access_schema, $reports_info['id'], $listing_split->current_page_number, $listing->listing_type);
        ...
        if($field['is_heading'] == 1)
        {
            ...
            $data_sort_url = (users::has_access('create') ? ' data-tt-sort-url="' . url_for('items/sort_nested','path=' . $path . '&redirect_to=' . $app_redirect_to . '&gotopage[' . _POST('reports_id') . ']=' . _POST('page')). '" ':'');

            $html .= '
                <td class="' . $field['type'] . $editable_listing->td_css_class() . '  field-' . $field['id'] . '-td item_heading_td' . (($listing->settings->get('heading_width_based_content') == 1 or $listing->settings->get('change_col_width_in_listing') == 1) ? ' width-auto' : '') . '" ' . $editable_listing->td_params() . '>
                    <div class="tt" 
                        ' . $data_sort_url  . ' data-tt-id="item_' . _POST('reports_id') . '_' . $item['id']. '" ' . ($item['parent_id']>0 ? ' data-tt-parent="item_' . _POST('reports_id') . '_' . $item['parent_id'] . '"':'') . '>
                    </div>
                    <a ' . $popup_html . ' class="item_heading_link" href="' . url_for('items/info', 'path=' . $path . '&redirect_to=subentity&gotopage[' . $_POST['reports_id'] . ']=' . $_POST['page']) . '">' . strip_tags(fields_types::output($output_options)) . '</a>
                ';
