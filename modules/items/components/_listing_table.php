    ...
    <table class="table table-striped table-bordered table-hover ' . ($listing->is_resizable() ? 'table-resizable' : '') . '" data-count-fixed-columns="' . reports::get_count_fixed_columns($_POST['reports_id'], $has_with_selected) . '" data-fixed-head="1" data-resizable="' . $listing->is_resizable() . '" ' . $listing->resizable_table_widht() . '>
      <thead>
        <tr class="listing-table-tr">
          ' . ($has_with_selected ? '<th class="multiple-select-action-th">' . $listing->select_all_choices($listing_split->number_of_pages) . '</th>' : '');
...
{
    $html .= '
      <tr class="listing-table-tr ' . (($users_notifications->has($item['id']) and $entity_cfg->get('disable_highlight_unread') != 1) ? 'unread-item-row' : '') . $listing_highlight->apply($item) . '">
        ';
    ...
    foreach($listing_fields as $field)
    {
        ...
        if($field['is_heading'] == 1)
        {
            ...
            $html .= '
                <td class="' . $field['type']  . $editable_listing->td_css_class() . '  field-' . $field['id'] . '-td item_heading_td' . (($entity_cfg->get('heading_width_based_content') == 1 or $entity_cfg->get('change_col_width_in_listing') == 1) ? ' width-auto' : '') . '" ' . $editable_listing->td_params() . '>
                    <a ' . $popup_html . ' class="item_heading_link" href="' . url_for('items/info', 'path=' . $path . '&redirect_to=subentity&gotopage[' . $_POST['reports_id'] . ']=' . $_POST['page']) . '">' . fields_types::output($output_options) . '</a>
            ';
