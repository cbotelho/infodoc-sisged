...
/* CRM - INFODOC-SISGED | 2026 https://ecmsolucoes.com */
  ...
  function load_items_listing(listing_container,page)
  {
    ...
    $('#'+listing_container).load('<?php echo url_for("items/listing")?>',
      ...
      function(response, status, xhr) {
        ...
        app_handle_listing_horisontal_scroll_float($(this))
        ...
        $('#'+listing_container+' .editable-cell').click(function(e){            
            if (e.target.tagName.toLowerCase() != 'a' && e.target.tagName.toLowerCase() != 'sup')
            {
                ...
                open_dialog("<?php echo url_for('items/form_single_field')?>&path="+$(this).attr('data_path')+'&field_id='+$(this).attr('data_field_id')+'&report_id='+$(this).attr('data_report_id')+'&page='+$(this).attr('data_page'))
        ...
        //apply custom column size
        app_handle_listing_resizer($(this),url_for('reports/reports','action=set_listing_col_width&reports_id='+listing_data[0]))
  ...
  {
      $('#'+listing_container+' .select_all_items').prop('checked',true);
      $('#'+listing_container+' .select_all_items_current_page').prop('checked',false);
      $('#'+listing_container+' .select_all_items').click()
  }
  ...
  {  
      ...
      $('#'+listing_container+' .select_all_items_current_page').click(function(){
            ...
            {      
                ...
                $('#'+listing_container).append('<div class="data_listing_processing"></div>').css("opacity", 0.5);                     
                
                $.ajax({type: "POST",url: '<?php echo url_for("items/select_items","action=select_on_page")?>',data: {items:items,reports_id: reports_id,path:listing_data[1]}}).done(function(){
                                        
                    $('#'+listing_container).css("opacity", 1);
                    $('#'+listing_container+' .data_listing_processing').remove();
            ...
            else
            {
                ...
                $.ajax({type: "POST",url: '<?php echo url_for("items/select_items","action=deselect_on_page")?>',data: {items:items,reports_id: reports_id,path:listing_data[1]}});
            }
      })
    ...
    $('#'+listing_container+' .items_checkbox').click(function(){   
      ...
      $.ajax({type: "POST",url: '<?php echo url_for("items/select_items","action=select")?>',data: {id:$(this).val(),checked: $(this).attr('checked'),reports_id: reports_id,path:listing_data[1]}});
    })
