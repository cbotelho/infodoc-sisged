...
switch($app_module_action)
{
    case 'sign':
        ...
        //check if sign exist
        $check_query = db_query("select id from  app_ext_signed_items where fields_id='" . _GET('fields_id') . "' and entities_id='" . $current_entity_id . "' and items_id='" . $current_item_id . "' and users_id='" . $app_user['id'] . "'");
        ...
        //get user cert
        $cryptopro_certificates_query = db_query("select certbase64,thumbprint from app_ext_cryptopro_certificates where users_id='" . $app_user['id']. "'");
        ...
        if($sign_file_signature)
        {
            ...
            {                                
                ...
                if(!cryptopro::SignatureCheck(file_get_contents($file['file_path']),$_POST['sign_file_signature'][$key],1,0))
                {
                    $alerts->add(TEXT_ERROR . ' ' . $SignatureCheckResult,'error');
                    redirect_to('items/info','path=' . $app_path);
                }
        ...
        try
        {
            ...
            if ($res)
            {
                ...
                if(!isset($CertData['subject']['SN'])) $CertData['subject']['SN'] = '';
                if(!isset($CertData['subject']['GN'])) $CertData['subject']['GN'] = '';
                if(!isset($CertData['subject']['title'])) $CertData['subject']['title'] = '';
                if(!isset($CertData['subject']['SNILS'])) $CertData['subject']['SNILS'] = '';
                if(!isset($CertData['subject']['INN'])) $CertData['subject']['INN'] = '';
                if(!isset($CertData['subject']['OGRN'])) $CertData['subject']['OGRN'] = '';
                if(!isset($CertData['subject']['ST'])) $CertData['subject']['ST'] = '';
                if(!isset($CertData['subject']['L'])) $CertData['subject']['L'] = '';
                if(!isset($CertData['subject']['O'])) $CertData['subject']['O'] = '';
                if(!isset($CertData['subject']['street'])) $CertData['subject']['street'] = '';
                
                if(!isset($CertData['issuer']['ST'])) $CertData['issuer']['ST'] = '';
                if(!isset($CertData['issuer']['street'])) $CertData['issuer']['street'] = '';
        ...
        $name = (strlen($CertData['subject']['SN']) ? $CertData['subject']['SN'] ." ". $CertData['subject']['GN'] : $CertData['subject']['CN']);
        ...
        if($cfg->get('run_process')>0)
        {
            ...
            if($cfg->get('assigned_to')>0 and isset($app_fields_cache[$current_entity_id][$cfg->get('assigned_to')]))
            {                
                $item_query = db_query("select  e.field_" . $cfg->get('assigned_to') . " as assigned_users from app_entity_" . $current_entity_id . " e where id='" . $current_item_id . "'");
                if($item = db_fetch_array($item_query))
                {
                    foreach(explode(',',$item['assigned_users']) as $user_id)
                    {
                        $check_query = db_query("select id from app_ext_signed_items where fields_id='" . _GET('fields_id') . "' and entities_id='" . $current_entity_id . "' and items_id='" . $current_item_id . "' and users_id='" . $user_id . "'");
                        if(!$check = db_fetch_array($check_query))
                        {
            ...
            if($all_users_singed)
            {
                redirect_to('items/processes','action=run&id=' . $cfg->get('run_process') . '&path=' . $app_path . '&redirect_to=items_info');
            }
        }
        else
        {
