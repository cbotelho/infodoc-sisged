...
{	
	switch($fields['type'])
	{
		...
		case 'fieldtype_entity':
			...
			{
				$obj['field_' . $fields['id']] = ((isset($_GET['current_field_values']) and strlen($_GET['current_field_values'])) ? $_GET['current_field_values'] . ',' : '') . _get::int('item_id');
			}
			else
			{
			...
			echo '
			    <script>
			        ...
			        app_handle_submodal_open_btn("field_' . $fields['id'] . '_row");
                        ...
                        $field_query = db_query("select * from app_fields where entities_id='" . $fields['entities_id'] . "' and type='fieldtype_entity_multilevel'");
			...
			switch($fields['type'])
			{
                            case 'fieldtype_users_ajax':
                                echo '
                                    <script>
			                $("#fields_' . $fields['id'] . '_select2_on").load("' . url_for('dashboard/select2_json','action=copy_values&form_type=items/render_field_value&entity_id=1&field_id=' . $fields['id']) . '",{item_id:' . _get::int('item_id') . '})
			            </script>
                                    ';
                                break;
			    case 'fieldtype_entity_ajax':
