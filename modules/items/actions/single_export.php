        ...
        if(isset($_POST['fields']))
        {
            ...
            while($tabs = db_fetch_array($tabs_query))
            {
                ...
                while($field = db_fetch_array($fields_query))
                {
                    ...
                    {
                        if(in_array($field['type'], array('fieldtype_input_numeric','fieldtype_formula','fieldtype_js_formula','fieldtype_user_photo', 'fieldtype_image', 'fieldtype_image_ajax', 'fieldtype_textarea_wysiwyg', 'fieldtype_textarea', 'fieldtype_barcode', 'fieldtype_qrcode', 'fieldtype_todo_list', 'fieldtype_text_pattern', 'fieldtype_text_pattern_static')))
                        {
                            if($_POST['export_type']=='pdf' and in_array($field['type'], array('fieldtype_user_photo', 'fieldtype_image', 'fieldtype_image_ajax')))
                            {
                                $file = attachments::parse_filename($value);
                                
                                if ($file['is_image'])
                                {
                                    $output =  '<img src="' . $file['file_path'] . '" width="250" vspace="0" hspace="0" style="margin-right: 10px; margin-bottom: 10px;">';
                                }
                            }
                            else
                            {
