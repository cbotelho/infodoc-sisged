...
while($item = db_fetch_array($items_query))
{
  ...
  if($entity_cfg->get('disable_avatar_in_comments',0)!=1 and $item['created_by']>0 and isset($app_users_cache[$item['created_by']]))
  {
      ...
      $message_css = 'message-margin';
      $message_arrow = '<span class="arrow"></span>';
  }
  ...
  $html .= '
        <li class="' . ($count/2==floor($count/2) ? 'out':'in') . '">
            ' . $photo . '
            <div class="message ' . $message_css . '" >
                ' . $message_arrow . '
                <div class="head">' . $html_action_column  . '                    
                    <span class="datetime">' . format_date_time($item['date_added'])  . ($entity_cfg->get('display_comments_id')==1 ? ' #' . $item['id']:''). '</span><br>    
                    ' . $name . '                    
                </div>    
