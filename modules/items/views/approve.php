...
if($cfg->get('use_signature')==1)
{
	...
	$content .= '<iframe width="100%" height="400" scrolling="no" frameborder="no" src="' . url_for('items/signature','fields_id=' . _get::int('fields_id') . '&path=' . $app_path . '&redirect_to=' . $app_redirect_to). '"></iframe>';
...
<?php echo form_tag('approve_form', url_for('items/approve','action=approve&fields_id=' . _get::int('fields_id') . '&path=' . $app_path)) ?>
