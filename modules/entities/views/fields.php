...
<div class="btn-group">
	...
	<ul class="dropdown-menu" role="menu">
		<li>
			<?php echo link_to_modalbox(TEXT_FIELDS_EXPORT,url_for('entities/fields_export_form','entities_id=' . $_GET['entities_id'])) ?>
		</li>
		<li>
...
<?php echo button_tag('<i class="fa fa-upload"></i>',url_for('entities/fields_import_form','entities_id=' . $_GET['entities_id']),true,['class'=>'btn btn-default','title'=>TEXT_IMPORT_FIELDS]) ?>
...
<?php }else{ ?>
  ...
  <td><?php echo fields_types::render_field_name($v['name'],$v['type'],$v['id']) . $heading_note ?></td>
  <td><?php echo $v['short_name']?></td>
  <td><?php echo tooltip_icon($v['notes'],'left') ?></td>  
  <td><?php echo render_bool_value($v['is_required'],true) ?></td>  
  <td><?php echo render_bool_value(($cfg->get('is_unique')>0 ? true:false),true) ?></td>  
  <td class="nowrap"><?php echo fields_types::get_title($v['type']) ?></td>
