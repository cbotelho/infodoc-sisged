...
if(isset($_POST['delete_login_page_background_mobile']))
{
  if(is_file(DIR_FS_UPLOADS . CFG_APP_LOGIN_PAGE_BACKGROUND_MOBILE))
  {
    unlink(DIR_FS_UPLOADS . CFG_APP_LOGIN_PAGE_BACKGROUND_MOBILE);
  }
  
  $_POST['CFG']['APP_LOGIN_PAGE_BACKGROUND_MOBILE'] = '';
}
...
if(isset($_POST['CFG']))
{
  foreach($_POST['CFG'] as $k=>$v)
  {
    ...
    if($k=='CFG_APP_LOGIN_PAGE_BACKGROUND_MOBILE')
    {                                                                            
      if(strlen($_FILES['APP_LOGIN_PAGE_BACKGROUND_MOBILE']['name'])>0)
      {                        
        if(is_image($_FILES['APP_LOGIN_PAGE_BACKGROUND_MOBILE']['tmp_name']))
        {
          $pathinfo = pathinfo($_FILES['APP_LOGIN_PAGE_BACKGROUND_MOBILE']['name']);
          $filename = 'app_bg_' . time() . '.' . $pathinfo['extension'];
          
          move_uploaded_file($_FILES['APP_LOGIN_PAGE_BACKGROUND_MOBILE']['tmp_name'], DIR_FS_UPLOADS  . $filename);
          $v = $filename;
        }
      }      
    }
