...
switch($app_module_action)
{
    case 'select_items':
        ...
        if (is_array($cfg->get('use_groups')) and count($cfg->get('use_groups')))
        {
            $filter_by_user_groups = $cfg->get('use_groups');
        }
                       
        if (count($filter_by_user_groups))
        {
            $listing_sql_query .= " and (e.field_6 in (" . db_input_in($filter_by_user_groups) . ")";
            foreach($filter_by_user_groups as $id)
            {
                $listing_sql_query .= " or find_in_set({$id}, multiple_access_groups)";
            }
        ...
        $listing_sql_query .= " and ((select count(*) from app_entities_access where (access_groups_id = e.field_6 or find_in_set(access_groups_id, e.multiple_access_groups)) and entities_id={$field['entities_id']} and length(access_schema)>0)>0";
        
        //add admins
        if ($cfg->get('hide_admin')!=1)
        {
            $listing_sql_query .= " or e.field_6=0 or find_in_set(0, multiple_access_groups)";
        }
