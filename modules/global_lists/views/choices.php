...
echo    '<div class="btn-group">' . 
        ...
        <ul class="dropdown-menu" role="menu">
            <li>
                    <a href="javascript: open_dialog(\'' . url_for('global_lists/choices_sort_reset','lists_id=' . $_GET['lists_id']) . '\')">' . TEXT_RESET_SORTING . '</a>
            </li>
        </ul>
        </div>
      ';
echo button_tag(TEXT_BUTTON_IMPORT, url_for('global_lists/choices_import', 'lists_id=' . $_GET['lists_id']), true, array('class' => 'btn btn-default'));
?>
...
<div class="btn-group">
    ...
    <ul class="dropdown-menu" role="menu">
        ...
        <li>
            <?php echo link_to_modalbox(TEXT_EXPORT, url_for('global_lists/choices_export', 'lists_id=' . $_GET['lists_id'])) ?>
        </li>
        <li>
        ...
        <tbody>
            ...
            foreach($tree as $v):
                ...
                <tr>
                    ...
                    <td>
                        <?php echo '<div class="tt" data-tt-id="choice_' . $v['id']. '" ' . ($v['parent_id']>0 ? 'data-tt-parent="choice_' . $v['parent_id'] . '"':''). ' data-tt-sort-url="' . url_for('global_lists/choices_sort', 'lists_id=' . $_GET['lists_id'] . '&parent_id=' . $v['id']) . '"></div>' ?>
                        <?php echo (strlen($v['icon']) ? app_render_icon($v['icon']) . ' ' : '') . $v['name'] . ' ' . tooltip_icon($v['notes'], 'right') ?>
                    </td>
