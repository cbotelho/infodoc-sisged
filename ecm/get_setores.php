...
if (isset($_GET['secretaria_id'])) {
    ...
    $stmt = $pdo->prepare("SELECT id, field_249 FROM app_entity_27 WHERE parent_item_id = ?");
    $stmt->execute([$secretariaId]);

    $options = '<option value="">Selecione o Setor</option>';
    while ($row = $stmt->fetch()) {
        $options .= '<option value="' . htmlspecialchars($row['id']) . '">' . htmlspecialchars($row['field_249']) . '</option>';
    }
