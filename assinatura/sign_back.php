...
// --- Upload de Certificado ---
    ...
    try {
        if (!isset($_FILES['certFile']) || $_FILES['certFile']['error'] !== UPLOAD_ERR_OK) {
            throw new Exception('Nenhum arquivo de certificado enviado ou erro no upload.');
        }
        $file = $_FILES['certFile'];
        $allowedExtensions = ['pfx','p12','pem','cer','crt'];
        ...
        if (!is_dir($certDir)) {
            if (!mkdir($certDir, 0755, true)) {
                throw new Exception('Falha ao criar diretório de certificados.');
            }
        }
...
// --- Processar Assinatura ---
    ...
    try {
        // LOG DIAGNÓSTICO: Salvar $_FILES e $_POST em cada requisição
        ...
        for ($i = 1; $i <= $pageCount; $i++) {
            ...
            if ($i == $targetPage) {
                ...
                $pdf->SetXY($x, $y);
                $pdf->MultiCell($w, 6,
                    "ECM - TECNOLOGIA E SOLUCOES LTDA:08505672000160\n".
                    "Assinado de forma digital por ECM - TECNOLOGIA E SOLUCOES LTDA:08505672000160\n".
                    "Dados: " . date('Y.m.d H:i:s') . " -03'00'",
                    0, 'L');
            }
        }
